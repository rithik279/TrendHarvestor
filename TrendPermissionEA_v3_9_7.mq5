//+------------------------------------------------------------------+
//|                                      TrendPermissionEA_v3_9_7.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer    |
//|                                                     Version 3.9.7 |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "3.9.7"
#property description "Asymmetric Basket TP + Fixed Grid + HTF ADX + Expansion Maximizer"
...
input group "06. v3.9.7 Expansion Maximizer"
input bool   EnableExpansionMultiplier = true;
input double ExpansionMultiplier      = 2.0;
input double ADXExpansionBuffer       = 5.0;
input double MaxBasketLots            = 0.08;
...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v3_9_7 initialized");
    Print("Symbol: ", _Symbol, " | Timeframe: M1");
    Print("----------------------------------------------");
    Print("LAYER 0 v3.9.7 - ASYMMETRIC BASKET TP + GRID + ADX + EXPANSION MAXIMIZER");
    ...
    Print("v3.9.7: Expansion Maximizer | Mult=", DoubleToString(ExpansionMultiplier, 2),
           " ADXBuf=", DoubleToString(ADXExpansionBuffer, 1),
           " MaxLots=", DoubleToString(MaxBasketLots, 2),
           " En=", EnableExpansionMultiplier);
...
{
    double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v3_9_7_BUY"))
        return true;
...
{
    double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v3_9_7_SELL"))
        return true;
...
//+------------------------------------------------------------------+
//| v3.9.7: Calculate current basket total lots                      |
//+------------------------------------------------------------------+
double CalculateBasketLots()
{
...
//+------------------------------------------------------------------+
//| v3.9.7: Determine if expansion regime is active for grid sizing  |
//+------------------------------------------------------------------+
bool IsExpansionRegime()
{
    if(!EnableExpansionMultiplier)
        return false;
    
    // ADX must be significantly above threshold
    if(g_adxValue <= (ADX_Threshold + ADXExpansionBuffer))
        return false;
    
    // EMA gap widening: emaGap > emaGapPrev AND gapSlope > 0
    if(!(emaGap > emaGapPrev && gapSlope > 0))
        return false;
    
    // ATR expanding: current ATR > previous ATR
    if(ArraySize(atrValues) < 3)
        return false;
    if(!(atrValues[1] > atrValues[2]))
        return false;
...
//| v3.9.7: Expansion-aware lot sizing + basket lot cap              |
//+------------------------------------------------------------------+
void ManageGridAdds()
{
    ...
    // v3.9.7: Determine lot size for this grid add
    bool expansionActive = IsExpansionRegime();
    double nextLot = expansionActive ? LotSize * ExpansionMultiplier : LotSize;
    
    // v3.9.7: Basket lot cap check
    double currentBasketLots = CalculateBasketLots();
    if(currentBasketLots + nextLot > MaxBasketLots)
    {
        if(EnableDebugPrints)
            Print(">>> v3.9.7 GRID ADD BLOCKED | BasketLots: ", DoubleToString(currentBasketLots, 2),
                  " + nextLot: ", DoubleToString(nextLot, 2),
                  " > MaxBasketLots: ", DoubleToString(MaxBasketLots, 2));
        return;
    }
    ...
    if(direction == BASKET_LONG)
    {
        ...
        if(bid >= lastAddAnchorPrice + FixedGridGap_USD)
        {
            ...
            double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            if(trade.Buy(nextLot, _Symbol, ask, 0, 0, "TrendPerm_v3_9_7_GRID_BUY"))
            {
                lastAddAnchorPrice = ask;
                if(EnableDebugPrints)
                {
                    ...
                    if(expansionActive)
                        Print(">>> v3.9.7 EXPANSION ADD | Multiplied Lot: ", DoubleToString(nextLot, 2),
                              " | ADX: ", DoubleToString(g_adxValue, 2),
                              " | BasketLots: ", DoubleToString(currentBasketLots + nextLot, 2));
                }
            }
        }
    }
    else if(direction == BASKET_SHORT)
    {
        ...
        if(ask <= lastAddAnchorPrice - FixedGridGap_USD)
        {
            double prevAnchor = lastAddAnchorPrice;
            double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            if(trade.Sell(nextLot, _Symbol, bid, 0, 0, "TrendPerm_v3_9_7_GRID_SELL"))
            {
                lastAddAnchorPrice = bid;
                if(EnableDebugPrints)
                {
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v3.9.7", C'100,180,255', 12, PANEL_FONT_BOLD);
...
void OnDeinit(const int reason)
{
    DeletePanel();
    Comment("");
    Print("TrendPermissionEA_v3_9_7 deinitialized. Reason: ", reason);
