//+------------------------------------------------------------------+
//|                                        TrendPermissionEA_v3_9.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer    |
//|                                                       Version 3.9 |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "3.9"
#property description "Asymmetric Basket Take-Profit + Fixed Grid Pyramiding"
...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v3_9 initialized");
    Print("Symbol: ", _Symbol, " | Timeframe: M1");
    Print("----------------------------------------------");
    Print("LAYER 0 v3.9 - ASYMMETRIC BASKET TP + GRID PYRAMIDING");
    Print("NEW: Per-position TP DISABLED, basket-level exits only");
    Print("NEW: Fixed Grid Pyramiding (gap: $", DoubleToString(FixedGridGap_USD, 2), ")");
    Print("NEW: Daily Profit Floor at $", DoubleToString(DailyProfitTarget_USD, 2));
...
{
    double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v3_9_BUY"))
        return true;
...
{
    double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v3_9_SELL"))
        return true;
...
void ManageGridAdds()
{
    ...
    if(direction == BASKET_LONG)
    {
        ...
        if(bid >= lastAddAnchorPrice + FixedGridGap_USD)
        {
            ...
            double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v3_9_GRID_BUY"))
            {
                lastAddAnchorPrice = ask;
                if(EnableDebugPrints)
                {
    ...
    else if(direction == BASKET_SHORT)
    {
        ...
        if(ask <= lastAddAnchorPrice - FixedGridGap_USD)
        {
            double prevAnchor = lastAddAnchorPrice;
            double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v3_9_GRID_SELL"))
            {
                lastAddAnchorPrice = bid;
                if(EnableDebugPrints)
                {
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v3.9", C'100,180,255', 12, PANEL_FONT_BOLD);
...
void OnDeinit(const int reason)
{
    DeletePanel();
    Comment("");
    Print("TrendPermissionEA_v3_9 deinitialized. Reason: ", reason);
