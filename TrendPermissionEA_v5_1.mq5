//+------------------------------------------------------------------+
//|                                      TrendPermissionEA_v5_1.mq5   |
//|                        Layer 0 Risk Governor + Execution Layer    |
//|                                                     Version 5.1   |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "5.1"
#property description "v5.1 – M5 Timeframe Port of v3.9.4"
...
int OnInit()
{
    ...
    {
        Print("ERROR: This EA is designed for M5 timeframe only.");
        return(INIT_FAILED);
    }
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v5_1 initialized");
    Print("Symbol: ", _Symbol, " | Timeframe: M5");
    Print("----------------------------------------------");
    Print("LAYER 0 v5.1 - M5 PORT OF v3.9.4");
    Print("v5.1: All trend permission + execution logic on M5");
...
void OnTick()
{
    ...
    // v5.1: New bar detection on M5 (PERIOD_CURRENT = M5 since EA enforces M5 chart)
...
//+------------------------------------------------------------------+
//| TREND PERMISSION MODULE (FROZEN — v5.1: All indicators on M5)    |
//+------------------------------------------------------------------+
...
bool CalculateEMAs_Frozen()
{
    ...
    // v5.1: PERIOD_CURRENT resolves to M5 (enforced by OnInit guard)
...
{
    // v5.1: Completed bar [1] is the last closed M5 bar
...
{
    double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_1_BUY"))
        return true;
...
{
    double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_1_SELL"))
        return true;
...
void ManageGridAdds()
{
    ...
    if(direction == BASKET_LONG)
    {
        ...
        if(bid >= lastAddAnchorPrice + FixedGridGap_USD)
        {
            ...
            double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_1_GRID_BUY"))
            {
                lastAddAnchorPrice = ask;
                if(EnableDebugPrints)
                {
    ...
    else if(direction == BASKET_SHORT)
    {
        ...
        if(ask <= lastAddAnchorPrice - FixedGridGap_USD)
        {
            double prevAnchor = lastAddAnchorPrice;
            double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_1_GRID_SELL"))
            {
                lastAddAnchorPrice = bid;
                if(EnableDebugPrints)
                {
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v5.1 M5", C'100,180,255', 12, PANEL_FONT_BOLD);
...
void OnDeinit(const int reason)
{
    DeletePanel();
    Comment("");
    Print("TrendPermissionEA_v5_1 deinitialized. Reason: ", reason);
