//+------------------------------------------------------------------+
//|                                      TrendPermissionEA_v5_0_10.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer     |
//|                                                    Version 5.0.10  |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "5.0.10"
#property description "v5.0.10 – Final Sequencing & State Hardening"
...
input group "05. Overlay Engine Settings (Stage 4 - Full Execution)"
input bool   EnableOverlayEngine = false;
input double OverlayLotSize = 0.01;                    // Overlay lot size per position
input double OverlayActivationThreshold_USD = 20.0;
...
input int    OverlayMaxMinutes = 30;
...
// ============================
// OVERLAY ENGINE STATE (Stage 4 - Full Execution)
// ============================
bool     overlayActive = false;
int      overlayLevelCount = 0;
double   overlayFloatingPL = 0.0;
// v5.0.10 CHANGE: overlayPeakFloatingPL REMOVED (unused)
...
// Stage 3 v5.0.1: Tick-Level Exhaustion Detection State
...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v5_0_10 initialized");
    Print("Overlay Sync Mode: FULL");
    ...
    Print("----------------------------------------------");
    Print("LAYER 0 v5.0.10 - ASYMMETRIC BASKET TP + GRID PYRAMIDING + HTF ADX + OVERLAY ENGINE");
...
void CheckDayReset()
{
    ...
    if(nyDate != currentNYDate)
    {
        ...
        // v5.0.4 CHANGE: overlayDailyRealizedPL removed — overlay realized now in dailyRealizedPL
...
//+------------------------------------------------------------------+
//| v5.0.2: OVERLAY STATE RESET (Stage 4)                            |
//+------------------------------------------------------------------+
void ResetOverlayState()
{
    // v5.0.10 CHANGE: overlayActive removed — derived from OverlayHasPositions() each tick
    overlayLevelCount = 0;
    overlayPlannedDirection = BASKET_NONE;
    overlaySignalActive = false;
    overlayImpulseValid = false;
    overlaySignalBarTime = 0;           // v5.0.3 PATCH 3: Prevent ghost re-trigger
...
//+------------------------------------------------------------------+
//| v5.0.2: OVERLAY GRID PYRAMIDING (Stage 4)                        |
//+------------------------------------------------------------------+
void ManageOverlayGrid()
{
    ...
    // v5.0.3 PATCH 4: Safety lock — verify positions still exist
    if(!OverlayHasPositions())
    {
        // v5.0.10 CHANGE: overlayActive not manually flipped — derived next tick
        return;
    }
    ...
    if(ovDir == BASKET_SHORT)
    {
        ...
        if(ask >= lastOverlayAnchor + OverlayGridGap_USD)
        {
            ...
            // Temporarily set magic for overlay trade
            trade.SetExpertMagicNumber(MAGIC_OVERLAY);
            if(trade.Sell(OverlayLotSize, _Symbol, bid, 0, 0, "Overlay_v5_0_10_GRID_SELL"))
            {
                overlayLevelCount++;
    ...
    else if(ovDir == BASKET_LONG)
    {
        ...
        if(bid <= lastOverlayAnchor - OverlayGridGap_USD)
        {
            ...
            trade.SetExpertMagicNumber(MAGIC_OVERLAY);
            if(trade.Buy(OverlayLotSize, _Symbol, ask, 0, 0, "Overlay_v5_0_10_GRID_BUY"))
            {
                overlayLevelCount++;
...
//+------------------------------------------------------------------+
//| v5.0.9: OVERLAY STRUCTURAL STOP (hardened — uses overlaySwingReference) |
//+------------------------------------------------------------------+
void CheckOverlayStructuralStop()
{
    ...
    // v5.0.9 CHANGE: Use fixed overlaySwingReference captured at entry
    // instead of live overlayImpulseHigh/Low which may change after entry
    if(ovDir == BASKET_SHORT)
    {
        double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
        if(bid > overlaySwingReference + OverlayBasketStopBuffer_USD)
        {
            Print("!!! OVERLAY STRUCTURAL STOP | Dir=SHORT | Bid=", DoubleToString(bid, 2),
    ...
    else if(ovDir == BASKET_LONG)
    {
        ...
        {
            Print("!!! OVERLAY STRUCTURAL STOP | Dir=LONG | Ask=", DoubleToString(ask, 2),
                  " < SwingRef=", DoubleToString(overlaySwingReference, 2),
                  " - Buffer=", DoubleToString(OverlayBasketStopBuffer_USD, 2));
            CloseOverlayPositions("Overlay_Structural_Stop");
            ResetOverlayState();
...
//+------------------------------------------------------------------+
//| v5.0.2: OVERLAY BASKET PROFIT TARGET (Stage 4)                   |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Stage 3 v5.0.1: IMPULSE CANDLE DETECTION (once per new bar)      |
//+------------------------------------------------------------------+
bool DetectImpulseCandle()
{
    // v5.0.3 PATCH 3: Do not refresh impulse while overlay is active
    if(overlayActive)
        return false;
    ...
    // New bar detected — reset previous signal state
    overlaySignalBarTime = barTime;
    overlayImpulseValid  = false;
    overlaySignalActive  = false;
    overlayPlannedDirection = BASKET_NONE;
    
    // Analyse completed bar [1]
    ...
    // Impulse requires body >= 70% of range
    if(body < range * 0.7)
        return false;
...
//+------------------------------------------------------------------+
//| Stage 3 v5.0.1: TICK-LEVEL EXHAUSTION DETECTION (every tick)     |
//+------------------------------------------------------------------+
bool DetectTickExhaustion(int &direction)
{
    ...
    // Determine impulse polarity from completed bar [1]
    double o1 = iOpen(_Symbol, PERIOD_CURRENT, 1);
    double c1 = iClose(_Symbol, PERIOD_CURRENT, 1);
    ...
    if(isBullishImpulse)
    {
        ...
        // Phase 1: Price must have traded above impulse high (h0 captures intra-bar peak)
        // Phase 2: Current bid has retreated below impulse close
        if(h0 > overlayImpulseHigh && bid < overlayImpulseClose)
        {
            ...
            {
                direction = BASKET_SHORT;   // Exhaustion of bull impulse → short overlay
    ...
    else // bearish impulse
    {
        ...
        // Phase 1: Price must have traded below impulse low (l0 captures intra-bar trough)
        // Phase 2: Current ask has recovered above impulse close
        if(l0 < overlayImpulseLow && ask > overlayImpulseClose)
        {
            ...
            {
                direction = BASKET_LONG;    // Exhaustion of bear impulse → long overlay
                overlaySignalActive = true;
                overlayPlannedDirection = direction;
                return true;
            }
...
void OnTick()
{
    ...
    // ==========================================================
    // v5.0.10 OVERLAY ENGINE (Stage 4 - Final Sequencing & State Hardening)
    ...
    if(EnableOverlayEngine)
    {
        ...
        else
        {
            ...
            // v5.0.10 CHANGE: Time expiry hardened with overlayEntryTime > 0 guard
            if(overlayActive && overlayEntryTime > 0 &&
               (TimeCurrent() - overlayEntryTime) > OverlayMaxMinutes * 60)
            {
            ...
            // 2. Impulse detection (once per new bar)
            DetectImpulseCandle();
            
            // 3. Tick-level exhaustion detection + immediate entry
            // v5.0.6 CHANGE: Overlay only activates when trend basket under meaningful stress
            if(!overlayActive && basketFloatingPL <= -OverlayActivationThreshold_USD)
            {
                int exDirection = BASKET_NONE;
                if(DetectTickExhaustion(exDirection))
                {
                    // === OVERLAY ENTRY ===
                    // v5.0.3 PATCH 7: Use impulse extrema for swing reference
                    if(exDirection == BASKET_SHORT)
                        overlaySwingReference = overlayImpulseHigh;
                    else
                        overlaySwingReference = overlayImpulseLow;
                    ...
                    {
                        double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
                        entryOK = trade.Sell(OverlayLotSize, _Symbol, bid, 0, 0, "Overlay_v5_0_10_SELL");
                        entryPrice = bid;
                    }
                    else if(exDirection == BASKET_LONG)
                    {
                        double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
                        entryOK = trade.Buy(OverlayLotSize, _Symbol, ask, 0, 0, "Overlay_v5_0_10_BUY");
                        entryPrice = ask;
                    }
                    ...
                    if(entryOK)
                    {
                        // v5.0.10 CHANGE: overlayActive not manually set — derived next tick
                        overlayLevelCount = 1;
                        lastOverlayAnchor = entryPrice;
                        
                        Print(">>> OVERLAY ENTRY | Dir=", (exDirection == BASKET_LONG ? "LONG" : "SHORT"),
                              " | Price=", DoubleToString(entryPrice, 2),
                              " | SwingStop=", DoubleToString(overlaySwingReference, 2));
                    }
                    else
                    {
                        Print("!!! OVERLAY ENTRY FAILED | Dir=", exDirection, " | Error=", GetLastError());
                        ResetOverlayState();
                    }
                }
            }
            
            // v5.0.10 CHANGE: Execution reordered — stop before expansion
            // 4. Overlay structural stop (must precede grid expansion)
            CheckOverlayStructuralStop();
            
            // 5. Overlay basket profit target
            overlayFloatingPL = CalculateOverlayFloatingPL();
            CheckOverlayProfitTarget();
            
            // 6. Overlay grid pyramiding (only after stop/target checks pass)
            ManageOverlayGrid();
        }
    }
...
{
    double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_0_10_BUY"))
        return true;
...
{
    double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_0_10_SELL"))
        return true;
...
void ManageGridAdds()
{
    ...
    if(direction == BASKET_LONG)
    {
        ...
        if(bid >= lastAddAnchorPrice + FixedGridGap_USD)
        {
            ...
            double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_0_10_GRID_BUY"))
            {
                lastAddAnchorPrice = ask;
                if(EnableDebugPrints)
                {
    ...
    else if(direction == BASKET_SHORT)
    {
        ...
        if(ask <= lastAddAnchorPrice - FixedGridGap_USD)
        {
            double prevAnchor = lastAddAnchorPrice;
            double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_0_10_GRID_SELL"))
            {
                lastAddAnchorPrice = bid;
                if(EnableDebugPrints)
                {
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v5.0.10", C'100,180,255', 12, PANEL_FONT_BOLD);
    ...
    // === Stage 2: Overlay Engine Section ===
    y += 155;
    
    CreateBox(PREFIX + "OverlayBG", PANEL_X, y, PANEL_WIDTH, 30, CLR_SECTION_BG);
    CreateLabel(PREFIX + "OverlayTitle", PANEL_X + padding, y + 6, "◈ OVERLAY ENGINE", clrWhite, 11, PANEL_FONT_BOLD);
    y += 35;
    
    CreateBox(PREFIX + "OverlayContentBG", PANEL_X, y, PANEL_WIDTH, 90, CLR_CONTENT_BG);
    contentY = y + 10;
...
void UpdatePanel()
{
    ...
    // === Stage 2: Overlay Engine Display ===
    string ovStateText = overlayActive ? "● ACTIVE" : "○ IDLE";
    color ovStateColor = overlayActive ? clrLime : clrGray;
    ...
    ObjectSetString(0, PREFIX + "OvLevelsValue", OBJPROP_TEXT, IntegerToString(overlayLevelCount));
...
void OnDeinit(const int reason)
{
    ...
    DeletePanel();
    Comment("");
    Print("TrendPermissionEA_v5_0_10 deinitialized. Reason: ", reason);
