//+------------------------------------------------------------------+
//|                                      TrendPermissionEA_v5_0_9.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer    |
//|                                                     Version 5.0.9 |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "5.0.9"
#property description "v5.0.9 – Overlay Structural Stop Hardening"
...
double   overlayPeakFloatingPL = 0.0;
...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v5_0_9 initialized");
    ...
    Print("----------------------------------------------");
    Print("LAYER 0 v5.0.9 - ASYMMETRIC BASKET TP + GRID PYRAMIDING + HTF ADX + OVERLAY ENGINE");
...
void ManageOverlayGrid()
{
    ...
    if(ovDir == BASKET_SHORT)
    {
        ...
        if(ask >= lastOverlayAnchor + OverlayGridGap_USD)
        {
            ...
            if(trade.Sell(OverlayLotSize, _Symbol, bid, 0, 0, "Overlay_v5_0_9_GRID_SELL"))
            {
                overlayLevelCount++;
    ...
    else if(ovDir == BASKET_LONG)
    {
        ...
        if(bid <= lastOverlayAnchor - OverlayGridGap_USD)
        {
            ...
            trade.SetExpertMagicNumber(MAGIC_OVERLAY);
            if(trade.Buy(OverlayLotSize, _Symbol, ask, 0, 0, "Overlay_v5_0_9_GRID_BUY"))
            {
                overlayLevelCount++;
...
void OnTick()
{
    ...
    // ==========================================================
    // v5.0.4 OVERLAY ENGINE (Stage 4 - Full Execution + Hardening + Unified Risk)
    ...
    if(EnableOverlayEngine)
    {
        ...
        else
        {
            ...
            // v5.0.3 PATCH 6: Time expiry safety
            if(overlayActive && (TimeCurrent() - overlayEntryTime) > OverlayMaxMinutes * 60)
            {
            ...
            if(!overlayActive && basketFloatingPL <= -OverlayActivationThreshold_USD)
            {
                ...
                if(DetectTickExhaustion(exDirection))
                {
                    ...
                    {
                        double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
                        entryOK = trade.Sell(OverlayLotSize, _Symbol, bid, 0, 0, "Overlay_v5_0_9_SELL");
                        entryPrice = bid;
                    }
                    else if(exDirection == BASKET_LONG)
                    {
                        double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
                        entryOK = trade.Buy(OverlayLotSize, _Symbol, ask, 0, 0, "Overlay_v5_0_9_BUY");
                        entryPrice = ask;
                    }
            ...
            // 4. Overlay grid pyramiding
            ManageOverlayGrid();
            
            // 5. Overlay structural stop
            CheckOverlayStructuralStop();
            
            // 6. Overlay basket profit target
            overlayFloatingPL = CalculateOverlayFloatingPL();
...
{
    double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_0_9_BUY"))
        return true;
...
{
    double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_0_9_SELL"))
        return true;
...
void ManageGridAdds()
{
    ...
    if(direction == BASKET_LONG)
    {
        ...
        if(bid >= lastAddAnchorPrice + FixedGridGap_USD)
        {
            ...
            double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_0_9_GRID_BUY"))
            {
                lastAddAnchorPrice = ask;
                if(EnableDebugPrints)
                {
    ...
    else if(direction == BASKET_SHORT)
    {
        ...
        if(ask <= lastAddAnchorPrice - FixedGridGap_USD)
        {
            double prevAnchor = lastAddAnchorPrice;
            double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_0_9_GRID_SELL"))
            {
                lastAddAnchorPrice = bid;
                if(EnableDebugPrints)
                {
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v5.0.9", C'100,180,255', 12, PANEL_FONT_BOLD);
...
void OnDeinit(const int reason)
{
    ...
    DeletePanel();
    Comment("");
    Print("TrendPermissionEA_v5_0_9 deinitialized. Reason: ", reason);
