//+------------------------------------------------------------------+
//|                                      TrendPermissionEA_v5_0_2.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer    |
//|                                                     Version 5.0.2 |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "5.0.2"
#property description "v5.0.2 – Full Overlay Execution Engine (Independent Basket)"
...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v5_0_2 initialized");
    Print("Symbol: ", _Symbol, " | Timeframe: M1");
    Print("----------------------------------------------");
    Print("LAYER 0 v5.0.2 - ASYMMETRIC BASKET TP + GRID PYRAMIDING + HTF ADX + OVERLAY ENGINE");
...
void Layer0_RiskCheck()
{
    ...
    if(basketExists)
    {
        if(basketFloatingPL <= -BasketStopLoss_USD)
        {
            ...
            // v5.0.2: Synchronized overlay close on trend basket hard stop
            if(EnableOverlayEngine && overlayActive)
            {
                Print(">>> OVERLAY SYNC CLOSE | Reason: Trend basket hard stop");
                CloseOverlayPositions("Trend_Basket_Stop_Sync");
                ResetOverlayState();
            }
...
void ManageOverlayGrid()
{
    ...
    if(ovDir == BASKET_SHORT)
    {
        ...
        if(ask >= lastOverlayAnchor + OverlayGridGap_USD)
        {
            ...
            if(trade.Sell(OverlayLotSize, _Symbol, bid, 0, 0, "Overlay_v5_0_2_GRID_SELL"))
            {
                overlayLevelCount++;
    ...
    else if(ovDir == BASKET_LONG)
    {
        ...
        if(bid <= lastOverlayAnchor - OverlayGridGap_USD)
        {
            ...
            trade.SetExpertMagicNumber(MAGIC_OVERLAY);
            if(trade.Buy(OverlayLotSize, _Symbol, ask, 0, 0, "Overlay_v5_0_2_GRID_BUY"))
            {
                overlayLevelCount++;
...
void OnTick()
{
    ...
    // ==========================================================
    // v5.0.2 OVERLAY ENGINE (Stage 4 - Full Execution)
    ...
    if(EnableOverlayEngine)
    {
        ...
        if(!overlayActive)
        {
            ...
            {
                // === OVERLAY ENTRY ===
                overlaySwingReference = overlayImpulseClose;
                ...
                {
                    double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
                    entryOK = trade.Sell(OverlayLotSize, _Symbol, bid, 0, 0, "Overlay_v5_0_2_SELL");
                    entryPrice = bid;
                }
                else if(exDirection == BASKET_LONG)
                {
                    double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
                    entryOK = trade.Buy(OverlayLotSize, _Symbol, ask, 0, 0, "Overlay_v5_0_2_BUY");
                    entryPrice = ask;
                }
...
{
    double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_0_2_BUY"))
        return true;
...
{
    double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_0_2_SELL"))
        return true;
...
void ManageGridAdds()
{
    ...
    if(direction == BASKET_LONG)
    {
        ...
        if(bid >= lastAddAnchorPrice + FixedGridGap_USD)
        {
            ...
            double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_0_2_GRID_BUY"))
            {
                lastAddAnchorPrice = ask;
                if(EnableDebugPrints)
                {
    ...
    else if(direction == BASKET_SHORT)
    {
        ...
        if(ask <= lastAddAnchorPrice - FixedGridGap_USD)
        {
            double prevAnchor = lastAddAnchorPrice;
            double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_0_2_GRID_SELL"))
            {
                lastAddAnchorPrice = bid;
                if(EnableDebugPrints)
                {
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v5.0.2", C'100,180,255', 12, PANEL_FONT_BOLD);
...
void OnDeinit(const int reason)
{
    DeletePanel();
    Comment("");
    Print("TrendPermissionEA_v5_0_2 deinitialized. Reason: ", reason);
