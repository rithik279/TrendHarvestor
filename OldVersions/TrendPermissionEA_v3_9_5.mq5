//+------------------------------------------------------------------+
//|                                      TrendPermissionEA_v3_9_5.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer    |
//|                                                     Version 3.9.5 |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "3.9.5"
#property description "Asymmetric Basket Take-Profit + Fixed Grid Pyramiding + HTF ADX Regime Filter + ATR Extension Filter"
...
input group "05. Entry Extension Filter"
input bool   EnableExtensionFilter = true;
input double MaxExtensionATR = 1.0;   // Maximum allowed |Close - EMA10| / ATR
...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v3_9_5 initialized");
    Print("Symbol: ", _Symbol, " | Timeframe: M1");
    Print("----------------------------------------------");
    Print("LAYER 0 v3.9.5 - ASYMMETRIC BASKET TP + GRID PYRAMIDING + HTF ADX + ATR EXT");
    ...
    Print("v3.9.5: ATR-Normalized EMA Extension Filter (MaxExt=", DoubleToString(MaxExtensionATR, 2), ", Enabled=", EnableExtensionFilter, ")");
...
void ExecuteTradeLogic()
{
    ...
    if(!BasketHasPositions())
    {
        ...
        // === v3.9.5: ATR-NORMALIZED EMA EXTENSION FILTER ===
        if(EnableExtensionFilter && ArraySize(emaFast) >= 2 && ArraySize(atrValues) >= 2)
        {
            double completedClose_ext = iClose(_Symbol, PERIOD_CURRENT, 1);
            double distance = MathAbs(completedClose_ext - emaFast[1]);
            double normalizedExtension = 0.0;
            
            if(atrValues[1] > 0)
                normalizedExtension = distance / atrValues[1];
            
            if(normalizedExtension > MaxExtensionATR)
            {
                if(EnableDebugPrints)
                {
                    string dir = longEntryAllowed ? "LONG" : (shortEntryAllowed ? "SHORT" : "NONE");
                    Print(">>> v3.9.5 EXTENSION BLOCK | Dir=", dir,
                          " | NormExt=", DoubleToString(normalizedExtension, 2),
                          " | Max=", DoubleToString(MaxExtensionATR, 2));
                }
                return;
            }
        }
...
{
    double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v3_9_5_BUY"))
        return true;
...
{
    double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v3_9_5_SELL"))
        return true;
...
void ManageGridAdds()
{
    ...
    if(direction == BASKET_LONG)
    {
        ...
        if(bid >= lastAddAnchorPrice + FixedGridGap_USD)
        {
            ...
            double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v3_9_5_GRID_BUY"))
            {
                lastAddAnchorPrice = ask;
                if(EnableDebugPrints)
                {
    ...
    else if(direction == BASKET_SHORT)
    {
        ...
        if(ask <= lastAddAnchorPrice - FixedGridGap_USD)
        {
            double prevAnchor = lastAddAnchorPrice;
            double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v3_9_5_GRID_SELL"))
            {
                lastAddAnchorPrice = bid;
                if(EnableDebugPrints)
                {
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v3.9.5", C'100,180,255', 12, PANEL_FONT_BOLD);
...
void OnDeinit(const int reason)
{
    DeletePanel();
    Comment("");
    Print("TrendPermissionEA_v3_9_5 deinitialized. Reason: ", reason);
