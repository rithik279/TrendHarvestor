...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v3 initialized");
...
void Layer0_RiskCheck()
{
    ...
    if(hasOpenPosition)
    {
        ...
        if(profitFromStart > 0 && currentEquity <= givebackThreshold)
        {
...
bool OpenBuyTrade()
{
    ...
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v3_BUY"))
    {
        return true;
    }
    else
...
bool OpenSellTrade()
{
    ...
    // No SL, No TP set here - TP managed by dollar amount separately
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v3_SELL"))
    {
        return true;
    }
    else
...
void CreatePanel()
{
    ...
    CreateBox(PREFIX + "Layer0ContentBG", PANEL_X, y, PANEL_WIDTH, 175, CLR_CONTENT_BG);
    int contentY = y + 10;
...
void OnDeinit(const int reason)
{
    ...
    Print("TrendPermissionEA_v3 deinitialized. Reason: ", reason);
