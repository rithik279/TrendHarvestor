//+------------------------------------------------------------------+
//|                                        TrendPermissionEA_v3_2.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer    |
//|                                                       Version 3.2 |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "3.2"
#property description "Permission-Gated Execution with Redesigned Layer 0 Risk Governor"
...
//+------------------------------------------------------------------+
//| GLOBAL VARIABLES - LAYER 0: RISK GOVERNOR (REDESIGNED v3.2)      |
//+------------------------------------------------------------------+

// === Account-Level Daily Tracking ===
double dailyStartEquity;           // Equity at start of trading day
double dailyRealizedPL;            // Realized P&L for the day
bool   dailyLossLimitHit;          // TRUE = trading disabled for day
datetime currentSessionDate;       // Current trading day

// === Basket-Level Tracking ===
double basketStartEquity;          // Equity when current basket started
double basketPeakEquity;           // Peak equity during current basket
...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v3_2 initialized");
    Print("Symbol: ", _Symbol, " | Timeframe: M1");
    Print("Lot Size: ", LotSize);
    Print("Take Profit: $", TakeProfitDollars);
    Print("Magic Number: ", MagicNumber);
    Print("----------------------------------------------");
    Print("LAYER 0 v3.2 - REDESIGNED");
    Print("Basket Stop Loss: $", BasketStopLoss_USD, " (per basket)");
...
//|                                                                  |
//| ================================================================ |
//| ===== LAYER 0: GLOBAL RISK GOVERNOR (REDESIGNED v3.2) ========== |
//| ================================================================ |
//|                                                                  |
//+------------------------------------------------------------------+
...
void InitializeLayer0()
{
    ...
    // === Basket-Level Tracking ===
    basketStartEquity = currentEquity;
    basketPeakEquity = currentEquity;
    basketActive = false;
...
//| PRIORITY ORDER: 1) Daily Loss  2) Basket Stop  3) Peak Giveback |
//+------------------------------------------------------------------+
void Layer0_RiskCheck()
{
    double currentEquity = AccountInfoDouble(ACCOUNT_EQUITY);
    
    // === STEP 0: CHECK FOR NEW DAY (MANDATORY RESET) ===
    CheckDayReset();
    
    // === PRIORITY 1: ACCOUNT-LEVEL DAILY LOSS LIMIT ===
    // This OVERRIDES everything - if hit, trading is disabled for the day
    ...
    if(dailyPL <= -DailyLossLimit_USD)
    {
        if(!dailyLossLimitHit)
        {
            ...
            Print(">>> DAILY LOSS LIMIT HIT: $", DoubleToString(dailyPL, 2), " – Trading disabled for day");
            Print("Daily Start Equity: $", DoubleToString(dailyStartEquity, 2));
    ...
    // Only affects current basket - does NOT disable future trading
    if(hasOpenPosition && basketActive)
    {
        double basketDD = currentEquity - basketStartEquity;
        
        if(basketDD <= -BasketStopLoss_USD)
        {
            Print("!!! LAYER 0: BASKET STOP HIT !!!");
            Print(">>> BASKET STOP HIT: $", DoubleToString(basketDD, 2), " – Basket closed, trading still enabled");
            Print("Basket Start Equity: $", DoubleToString(basketStartEquity, 2));
            Print("Current Equity: $", DoubleToString(currentEquity, 2));
            
            CloseAllPositions("Layer0_BasketStop");
            ResetBasketState();
            
            // IMPORTANT: Do NOT disable trading - allow new baskets
            // tradingDisabled remains FALSE
            // disableReason shows last event but trading continues
        ...
        // Update peak equity while basket is active
        if(currentEquity > basketPeakEquity)
        {
            basketPeakEquity = currentEquity;
        }
        
        // Calculate profit from basket start
        double profitFromBasketStart = basketPeakEquity - basketStartEquity;
        
        // Giveback logic only activates after MinProfitToProtect threshold
        if(profitFromBasketStart >= MinProfitToProtect_USD)
        {
            // Log when protection becomes active (once per basket)
            if(!profitProtectionActivated)
            {
                profitProtectionActivated = true;
                Print(">>> LAYER 0: Profit protection activated at $", DoubleToString(profitFromBasketStart, 2), " peak profit");
            }
            
            // Check giveback threshold
            double allowedGiveback = PeakGivebackFrac * profitFromBasketStart;
            double givebackThreshold = basketPeakEquity - allowedGiveback;
            
            if(currentEquity <= givebackThreshold)
            {
                Print("!!! LAYER 0: PEAK GIVEBACK LIMIT HIT !!!");
                Print("Basket Peak Equity: $", DoubleToString(basketPeakEquity, 2));
                Print("Current Equity: $", DoubleToString(currentEquity, 2));
    ...
    if(dailyRealizedPL >= DailyProfitTarget_USD)
    {
        if(!tradingDisabled || disableReason != "DAILY TARGET REACHED")
        {
            ...
            Print("Daily Target: $", DailyProfitTarget_USD);
...
//+------------------------------------------------------------------+
//| Check for Day Reset (New Trading Day)                            |
//+------------------------------------------------------------------+
void CheckDayReset()
{
    ...
    if(brokerDate != currentSessionDate)
    {
        ...
        // Re-enable trading (new day = fresh start)
        tradingDisabled = false;
        disableReason = "";
        ...
        Print("==============================================");
        Print(">>> NEW DAY DETECTED – Daily limits reset");
...
//+------------------------------------------------------------------+
//| Start New Basket (called when opening first position)            |
//+------------------------------------------------------------------+
void StartNewBasket()
{
    double currentEquity = AccountInfoDouble(ACCOUNT_EQUITY);
    ...
    if(EnableDebugPrints)
        Print(">>> NEW BASKET STARTED | Equity: $", DoubleToString(currentEquity, 2));
}
...
void ExecuteTradeLogic()
{
    ...
    if(longEntryAllowed)
    {
        if(OpenBuyTrade())
        {
            ...
            // Start new basket if not already active
            if(!basketActive)
                StartNewBasket();
...
bool OpenBuyTrade()
{
    ...
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v3_2_BUY"))
    {
        return true;
    }
    else
...
bool OpenSellTrade()
{
    ...
    // No SL, No TP set here - TP managed by dollar amount separately
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v3_2_SELL"))
    {
        return true;
    }
    else
...
void CheckPositionStatus()
{
    ...
    // If we had a position and now don't, basket is no longer active
    if(hadPosition && !hasOpenPosition)
    {
        basketActive = false;
        
        // Clear basket-related status messages if not disabled
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v3.2", C'100,180,255', 12, PANEL_FONT_BOLD);
    ...
    // Basket DD (prominent)
    CreateLabel(PREFIX + "BasketDDLabel", PANEL_X + padding, contentY, "Basket DD:", CLR_LABEL, PANEL_FONT_SIZE, PANEL_FONT);
    CreateLabel(PREFIX + "BasketDDValue", valueX, contentY, "", CLR_VALUE, 11, PANEL_FONT_BOLD);
    contentY += rowHeight;
    ...
    CreateLabel(PREFIX + "DailyStartLabel", PANEL_X + padding, contentY, "Daily Start:", CLR_LABEL, PANEL_FONT_SIZE, PANEL_FONT);
    CreateLabel(PREFIX + "DailyStartValue", valueX, contentY, "", CLR_VALUE, PANEL_FONT_SIZE, PANEL_FONT_BOLD);
    contentY += rowHeight;
...
void UpdatePanel()
{
    ...
    double dailyPL = currentEquity - dailyStartEquity;
    double basketDD = basketActive ? (currentEquity - basketStartEquity) : 0.0;
    ...
    if(disableReason != "")
    {
        ...
        if(disableReason == "DAILY LOSS LIMIT HIT")
            statusColor = clrRed;
        else if(disableReason == "DAILY TARGET REACHED")
            statusColor = clrGold;
        else if(disableReason == "BASKET STOP HIT (Ready)" || disableReason == "PEAK GIVEBACK (Ready)")
    ...
    if(dailyPL >= 0)
        dailyColor = clrLime;
    else if(dailyPL > -DailyLossLimit_USD * 0.5)
        dailyColor = clrOrange;
    else
        dailyColor = clrRed;
    ...
    // === BASKET DD (with limit) ===
    string basketText;
    color basketColor;
    
    if(!basketActive && !hasOpenPosition)
    {
        basketText = "○ NO BASKET";
        basketColor = clrGray;
    }
    else
    {
        basketText = "$" + DoubleToString(basketDD, 2) + " / -$" + DoubleToString(BasketStopLoss_USD, 2);
        if(basketDD >= 0)
            basketColor = clrLime;
        else if(basketDD > -BasketStopLoss_USD * 0.5)
            basketColor = clrOrange;
        else
            basketColor = clrRed;
    }
    ObjectSetString(0, PREFIX + "BasketDDValue", OBJPROP_TEXT, basketText);
    ObjectSetInteger(0, PREFIX + "BasketDDValue", OBJPROP_COLOR, basketColor);
    
    // === EQUITY VALUES ===
    ObjectSetString(0, PREFIX + "DailyStartValue", OBJPROP_TEXT, "$" + DoubleToString(dailyStartEquity, 2));
...
void OnDeinit(const int reason)
{
    ...
    Print("TrendPermissionEA_v3_2 deinitialized. Reason: ", reason);
