//+------------------------------------------------------------------+
//|                                      TrendPermissionEA_v5_0_3.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer    |
//|                                                     Version 5.0.3 |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "5.0.3"
#property description "v5.0.3 – Overlay Hardening + State Integrity + Risk Sync"
...
// v5.0.3 PATCH 5: Explicit overlay daily realized P&L tracking
double   overlayDailyRealizedPL = 0.0;
...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v5_0_3 initialized");
    Print("Overlay Sync Mode: FULL");
    Print("Overlay Daily Tracking: ENABLED");
    Print("State Integrity Guards: ACTIVE");
    Print("Symbol: ", _Symbol, " | Timeframe: M1");
    Print("----------------------------------------------");
    Print("LAYER 0 v5.0.3 - ASYMMETRIC BASKET TP + GRID PYRAMIDING + HTF ADX + OVERLAY ENGINE");
...
void CheckDayReset()
{
    ...
    if(nyDate != currentNYDate)
    {
        ...
        // v5.0.3: Reset overlay daily realized P&L
        overlayDailyRealizedPL = 0.0;
...
void ManageOverlayGrid()
{
    ...
    if(ovDir == BASKET_SHORT)
    {
        ...
        if(ask >= lastOverlayAnchor + OverlayGridGap_USD)
        {
            ...
            if(trade.Sell(OverlayLotSize, _Symbol, bid, 0, 0, "Overlay_v5_0_3_GRID_SELL"))
            {
                overlayLevelCount++;
    ...
    else if(ovDir == BASKET_LONG)
    {
        ...
        if(bid <= lastOverlayAnchor - OverlayGridGap_USD)
        {
            ...
            trade.SetExpertMagicNumber(MAGIC_OVERLAY);
            if(trade.Buy(OverlayLotSize, _Symbol, ask, 0, 0, "Overlay_v5_0_3_GRID_BUY"))
            {
                overlayLevelCount++;
        ...
        if(trans.deal_type == DEAL_TYPE_BUY || trans.deal_type == DEAL_TYPE_SELL)
        {
            if(HistoryDealSelect(trans.deal))
            {
                ...
                if(entry == DEAL_ENTRY_OUT || entry == DEAL_ENTRY_INOUT)
                {
                    ...
                    // v5.0.3 PATCH 5: Explicit overlay realized P&L tracking
                    if(magic == MAGIC_OVERLAY)
                    {
                        overlayDailyRealizedPL += profit;
                        if(EnableDebugPrints)
                            Print(">>> OVERLAY DEAL CLOSED | Profit: $", DoubleToString(profit, 2), " | Overlay Daily Realized: $", DoubleToString(overlayDailyRealizedPL, 2));
                    }
                }
            }
        }
...
void OnTick()
{
    ...
    // ==========================================================
    // v5.0.3 OVERLAY ENGINE (Stage 4 - Full Execution + Hardening)
        ...
        else
        {
            ...
            if(!overlayActive)
            {
                ...
                if(DetectTickExhaustion(exDirection))
                {
                    ...
                    {
                        double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
                        entryOK = trade.Sell(OverlayLotSize, _Symbol, bid, 0, 0, "Overlay_v5_0_3_SELL");
                        entryPrice = bid;
                    }
                    else if(exDirection == BASKET_LONG)
                    {
                        double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
                        entryOK = trade.Buy(OverlayLotSize, _Symbol, ask, 0, 0, "Overlay_v5_0_3_BUY");
                        entryPrice = ask;
                    }
...
{
    double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_0_3_BUY"))
        return true;
...
{
    double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_0_3_SELL"))
        return true;
...
void ManageGridAdds()
{
    ...
    if(direction == BASKET_LONG)
    {
        ...
        if(bid >= lastAddAnchorPrice + FixedGridGap_USD)
        {
            ...
            double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_0_3_GRID_BUY"))
            {
                lastAddAnchorPrice = ask;
                if(EnableDebugPrints)
                {
    ...
    else if(direction == BASKET_SHORT)
    {
        ...
        if(ask <= lastAddAnchorPrice - FixedGridGap_USD)
        {
            double prevAnchor = lastAddAnchorPrice;
            double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_0_3_GRID_SELL"))
            {
                lastAddAnchorPrice = bid;
                if(EnableDebugPrints)
                {
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v5.0.3", C'100,180,255', 12, PANEL_FONT_BOLD);
...
void OnDeinit(const int reason)
{
    DeletePanel();
    Comment("");
    Print("TrendPermissionEA_v5_0_3 deinitialized. Reason: ", reason);
