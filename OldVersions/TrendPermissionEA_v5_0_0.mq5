//+------------------------------------------------------------------+
//|                                      TrendPermissionEA_v5_0_0.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer    |
//|                                                     Version 5.0.0 |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "5.0.0"
#property description "Asymmetric Basket Take-Profit + Fixed Grid Pyramiding + HTF ADX Regime Filter"
...
// Stage 3: Exhaustion detection state
datetime g_lastExhaustionBarTime = 0;
...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v5_0_0 initialized");
    Print("Symbol: ", _Symbol, " | Timeframe: M1");
    Print("----------------------------------------------");
    Print("LAYER 0 v5.0.0 - ASYMMETRIC BASKET TP + GRID PYRAMIDING + HTF ADX");
...
//+------------------------------------------------------------------+
//| Stage 3: EXHAUSTION DETECTION (no trade execution)               |
//+------------------------------------------------------------------+
bool DetectExhaustionEvent(int &direction)
{
    // Fire only once per completed bar
    datetime barTime = iTime(_Symbol, PERIOD_CURRENT, 1);
    if(barTime == g_lastExhaustionBarTime)
        return false;
    
    // === Impulse candle analysis on completed bar [1] ===
    double impulseOpen  = iOpen(_Symbol, PERIOD_CURRENT, 1);
    double impulseClose = iClose(_Symbol, PERIOD_CURRENT, 1);
    double impulseHigh  = iHigh(_Symbol, PERIOD_CURRENT, 1);
    double impulseLow   = iLow(_Symbol, PERIOD_CURRENT, 1);
    
    double body  = MathAbs(impulseClose - impulseOpen);
    ...
    bool isBullishImpulse = (impulseClose > impulseOpen);
    bool isBearishImpulse = (impulseClose < impulseOpen);
    
    // === Exhaustion check on forming bar [0] ===
    double currentOpen  = iOpen(_Symbol, PERIOD_CURRENT, 0);
    double currentHigh  = iHigh(_Symbol, PERIOD_CURRENT, 0);
    double currentLow   = iLow(_Symbol, PERIOD_CURRENT, 0);
    double currentBody   = 0.0;
    ...
    if(isBullishImpulse)
    {
        // Bullish exhaustion: bid retreated below impulse close + upper wick dominant
        double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
        double upperWick = currentHigh - MathMax(currentOpen, bid);
        double lowerWick = MathMin(currentOpen, bid) - currentLow;
        
        if(bid < impulseClose && upperWick > lowerWick)
        {
            direction = BASKET_SHORT;  // Exhaustion of bullish impulse → short overlay
            g_lastExhaustionBarTime = barTime;
            if(EnableDebugPrints)
                Print(">>> STAGE 3: BULLISH EXHAUSTION | ImpulseBody=", DoubleToString(body, 2),
                      " | Range=", DoubleToString(range, 2),
                      " | Bid=", DoubleToString(bid, 2),
                      " | ImpClose=", DoubleToString(impulseClose, 2));
            return true;
        }
    }
    else if(isBearishImpulse)
    {
        // Bearish exhaustion: ask recovered above impulse close + lower wick dominant
        double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
        double upperWick = currentHigh - MathMax(currentOpen, ask);
        double lowerWick = MathMin(currentOpen, ask) - currentLow;
        
        if(ask > impulseClose && lowerWick > upperWick)
        {
            direction = BASKET_LONG;  // Exhaustion of bearish impulse → long overlay
            g_lastExhaustionBarTime = barTime;
            if(EnableDebugPrints)
                Print(">>> STAGE 3: BEARISH EXHAUSTION | ImpulseBody=", DoubleToString(body, 2),
                      " | Range=", DoubleToString(range, 2),
...
void OnTick()
{
    ...
    // Stage 3: Exhaustion detection (no trades placed)
    if(EnableOverlayEngine && !overlayActive)
    {
        int exDirection = BASKET_NONE;
        if(DetectExhaustionEvent(exDirection))
        {
            overlaySwingReference = iClose(_Symbol, PERIOD_CURRENT, 1);
            overlayEntryTime = TimeCurrent();
            Print(">>> STAGE 3 EXHAUSTION DETECTED | Dir=", exDirection,
...
{
    double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_0_0_BUY"))
        return true;
...
{
    double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_0_0_SELL"))
        return true;
...
void ManageGridAdds()
{
    ...
    if(direction == BASKET_LONG)
    {
        ...
        if(bid >= lastAddAnchorPrice + FixedGridGap_USD)
        {
            ...
            double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_0_0_GRID_BUY"))
            {
                lastAddAnchorPrice = ask;
                if(EnableDebugPrints)
                {
    ...
    else if(direction == BASKET_SHORT)
    {
        ...
        if(ask <= lastAddAnchorPrice - FixedGridGap_USD)
        {
            double prevAnchor = lastAddAnchorPrice;
            double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_0_0_GRID_SELL"))
            {
                lastAddAnchorPrice = bid;
                if(EnableDebugPrints)
                {
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v5.0.0", C'100,180,255', 12, PANEL_FONT_BOLD);
...
void OnDeinit(const int reason)
{
    DeletePanel();
    Comment("");
    Print("TrendPermissionEA_v5_0_0 deinitialized. Reason: ", reason);
