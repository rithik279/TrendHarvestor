//+------------------------------------------------------------------+
//|                                      TrendPermissionEA_v5_0_7.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer    |
//|                                                     Version 5.0.7 |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "5.0.7"
#property description "v5.0.7 – Exposure Control (Overlay Blocks New Trend Entries)"
...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v5_0_7 initialized");
    ...
    Print("----------------------------------------------");
    Print("LAYER 0 v5.0.7 - ASYMMETRIC BASKET TP + GRID PYRAMIDING + HTF ADX + OVERLAY ENGINE");
...
void ManageOverlayGrid()
{
    ...
    if(ovDir == BASKET_SHORT)
    {
        ...
        if(ask >= lastOverlayAnchor + OverlayGridGap_USD)
        {
            ...
            if(trade.Sell(OverlayLotSize, _Symbol, bid, 0, 0, "Overlay_v5_0_7_GRID_SELL"))
            {
                overlayLevelCount++;
    ...
    else if(ovDir == BASKET_LONG)
    {
        ...
        if(bid <= lastOverlayAnchor - OverlayGridGap_USD)
        {
            ...
            trade.SetExpertMagicNumber(MAGIC_OVERLAY);
            if(trade.Buy(OverlayLotSize, _Symbol, ask, 0, 0, "Overlay_v5_0_7_GRID_BUY"))
            {
                overlayLevelCount++;
        ...
        else
        {
            ...
            if(!overlayActive && basketFloatingPL <= -OverlayActivationThreshold_USD)
            {
                ...
                if(DetectTickExhaustion(exDirection))
                {
                    ...
                    {
                        double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
                        entryOK = trade.Sell(OverlayLotSize, _Symbol, bid, 0, 0, "Overlay_v5_0_7_SELL");
                        entryPrice = bid;
                    }
                    else if(exDirection == BASKET_LONG)
                    {
                        double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
                        entryOK = trade.Buy(OverlayLotSize, _Symbol, ask, 0, 0, "Overlay_v5_0_7_BUY");
                        entryPrice = ask;
                    }
...
{
    double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_0_7_BUY"))
        return true;
...
{
    double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_0_7_SELL"))
        return true;
...
void ManageGridAdds()
{
    ...
    if(direction == BASKET_LONG)
    {
        ...
        if(bid >= lastAddAnchorPrice + FixedGridGap_USD)
        {
            ...
            double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_0_7_GRID_BUY"))
            {
                lastAddAnchorPrice = ask;
                if(EnableDebugPrints)
                {
    ...
    else if(direction == BASKET_SHORT)
    {
        ...
        if(ask <= lastAddAnchorPrice - FixedGridGap_USD)
        {
            double prevAnchor = lastAddAnchorPrice;
            double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_0_7_GRID_SELL"))
            {
                lastAddAnchorPrice = bid;
                if(EnableDebugPrints)
                {
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v5.0.7", C'100,180,255', 12, PANEL_FONT_BOLD);
...
void OnDeinit(const int reason)
{
    DeletePanel();
    Comment("");
    Print("TrendPermissionEA_v5_0_7 deinitialized. Reason: ", reason);
