//+------------------------------------------------------------------+
//|                                        TrendPermissionEA_v3_5.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer    |
//|                                                       Version 3.5 |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "3.5"
#property description "Peak protection using basket floating P&L (clarified)"
...
// === Basket-Level Tracking (v3.5: Clarified floating P&L based) ===
double basketFloatingPL;           // Current floating P&L of basket
double basketPeakFloatingPL;       // v3.5: Peak floating P&L (renamed for clarity)
bool   basketActive;
...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v3_5 initialized");
    Print("Symbol: ", _Symbol, " | Timeframe: M1");
    Print("----------------------------------------------");
    Print("LAYER 0 v3.5 - CLARIFIED PEAK PROTECTION");
    Print("Basket Stop Loss: $", BasketStopLoss_USD);
    Print("Daily Loss Limit: $", DailyLossLimit_USD);
    Print("Peak Giveback: ", PeakGivebackFrac * 100, "% of peak floating P&L");
    Print("Min Peak to Protect: $", MinProfitToProtect_USD);
    Print("Giveback Formula: close if floatingPL <= peakPL * (1 - ", PeakGivebackFrac, ")");
    Print("==============================================");
...
void InitializeLayer0()
{
    ...
    basketPeakFloatingPL = 0.0;  // v3.5: Renamed for clarity
    basketActive = false;
...
void Layer0_RiskCheck()
{
    ...
    if(hasOpenPosition && basketActive)
    {
        ...
        // === PRIORITY 3: PEAK PROFIT GIVEBACK (v3.5 - CLARIFIED) ===
        
        // v3.5: Track peak floating P&L
        basketPeakFloatingPL = MathMax(basketPeakFloatingPL, basketFloatingPL);
        
        // v3.5: Protection activates when peak floating P&L >= MinProfitToProtect_USD
        if(basketPeakFloatingPL >= MinProfitToProtect_USD)
        {
            if(!profitProtectionActivated)
            {
                ...
                Print("    Threshold was: $", DoubleToString(MinProfitToProtect_USD, 2));
            }
            
            // v3.5: Giveback threshold = peakPL * (1 - PeakGivebackFrac)
            double givebackThreshold = basketPeakFloatingPL * (1.0 - PeakGivebackFrac);
            double allowedGiveback = basketPeakFloatingPL - givebackThreshold;
            ...
            if(basketFloatingPL <= givebackThreshold)
            {
                ...
                Print("    Formula: floatingPL <= peakPL * (1 - ", PeakGivebackFrac, ")");
                Print("             $", DoubleToString(basketFloatingPL, 2), " <= $", DoubleToString(givebackThreshold, 2));
...
{
    basketFloatingPL = 0.0;
    basketPeakFloatingPL = 0.0;  // v3.5: Reset peak
    basketActive = false;
    profitProtectionActivated = false;
    
    if(EnableDebugPrints)
        Print(">>> BASKET STATE RESET | Peak: $0.00, Active: false, Protection: false");
}
...
void StartNewBasket()
{
    ...
    if(EnableDebugPrints)
        Print(">>> NEW BASKET STARTED | Peak tracking begins at $0.00");
}
...
bool OpenBuyTrade()
{
    ...
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v3_5_BUY"))
        return true;
...
bool OpenSellTrade()
{
    ...
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v3_5_SELL"))
        return true;
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v3.5", C'100,180,255', 12, PANEL_FONT_BOLD);
    ...
    CreateLabel(PREFIX + "GivebackLabel", PANEL_X + padding, contentY, "Giveback Threshold:", CLR_LABEL, PANEL_FONT_SIZE, PANEL_FONT);
    CreateLabel(PREFIX + "GivebackValue", valueX, contentY, "", CLR_VALUE, 11, PANEL_FONT_BOLD);
    contentY += rowHeight;
...
void UpdatePanel()
{
    ...
    // v3.5: Show giveback threshold
    string givebackText;
    color givebackColor;
    if(!basketActive && !hasOpenPosition)
    {
        givebackText = "○ NO BASKET";
        givebackColor = clrGray;
    }
    else if(basketPeakFloatingPL < MinProfitToProtect_USD)
    {
        givebackText = "○ INACTIVE (peak < $" + DoubleToString(MinProfitToProtect_USD, 2) + ")";
        givebackColor = clrGray;
    }
    else
    {
        double threshold = basketPeakFloatingPL * (1.0 - PeakGivebackFrac);
        givebackText = "$" + DoubleToString(threshold, 2) + " (" + DoubleToString((1.0 - PeakGivebackFrac) * 100, 0) + "% of peak)";
        givebackColor = clrOrange;
    }
    ObjectSetString(0, PREFIX + "GivebackValue", OBJPROP_TEXT, givebackText);
    ObjectSetInteger(0, PREFIX + "GivebackValue", OBJPROP_COLOR, givebackColor);
...
void OnDeinit(const int reason)
{
    DeletePanel();
    Comment("");
    Print("TrendPermissionEA_v3_5 deinitialized. Reason: ", reason);
