//+------------------------------------------------------------------+
//|                                      TrendPermissionEA_v3_9_2.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer    |
//|                                                     Version 3.9.2 |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "3.9.2"
#property description "Asymmetric Basket TP + Grid + EMA30 Invalidation + Permission Silence Exit"
...
// === v3.9.2: Permission Silence Counters (bar-based) ===
int    barsSinceLongPermissionBurst  = 0;
int    barsSinceShortPermissionBurst = 0;
#define MAX_PERMISSION_SILENCE_BARS 5
...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v3_9_2 initialized");
    Print("Symbol: ", _Symbol, " | Timeframe: M1");
    Print("----------------------------------------------");
    Print("LAYER 0 v3.9.2 - BASKET TP + GRID + EMA30 + SILENCE EXIT");
    Print("NEW: Permission Silence Exit (", MAX_PERMISSION_SILENCE_BARS, "-bar silence rule)");
    Print("KEPT: Structural EMA30 Invalidation Exit (v3.9.1)");
...
void InitializeLayer0()
{
    ...
    // v3.9.2: Permission silence counters
    barsSinceLongPermissionBurst = 0;
    barsSinceShortPermissionBurst = 0;
...
//+------------------------------------------------------------------+
//| v3.9.1: Count Basket Positions (helper for debug logging)        |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| v3.9.1: Structural EMA30 Invalidation Check                     |
//|                                                                   |
//| For LONG basket:                                                  |
//|   Exit if Close[1] < EMA30[1] AND Close[2] < EMA30[2]           |
//|          AND emaFastSlope <= 0                                    |
//|                                                                   |
//| For SHORT basket:                                                 |
//|   Exit if Close[1] > EMA30[1] AND Close[2] > EMA30[2]           |
//|          AND emaFastSlope >= 0                                    |
//|                                                                   |
//| Uses already-computed emaSlow[] and emaFastSlope.                 |
//| Does NOT recalculate EMAs.                                        |
//+------------------------------------------------------------------+
bool StructuralInvalidationTriggered(int basketDirection)
{
    // Safety: EMA arrays must be populated (at least 3 elements for [0], [1], [2])
    ...
    {
        // LONG invalidation: 2 consecutive closes below EMA30 + fast slope turned flat/down
        if(close1 < emaSlow[1] && close2 < emaSlow[2] && emaFastSlope <= 0)
            return true;
    }
    else if(basketDirection == BASKET_SHORT)
    {
        // SHORT invalidation: 2 consecutive closes above EMA30 + fast slope turned flat/up
        if(close1 > emaSlow[1] && close2 > emaSlow[2] && emaFastSlope >= 0)
            return true;
    }
...
//+------------------------------------------------------------------+
//| v3.9.2: Permission Silence Check                                 |
//|                                                                   |
//| For LONG basket:                                                  |
//|   Exit if barsSinceLongPermissionBurst >= MAX_SILENCE_BARS        |
//|                                                                   |
//| For SHORT basket:                                                 |
//|   Exit if barsSinceShortPermissionBurst >= MAX_SILENCE_BARS       |
//|                                                                   |
//| A "burst" is a 0→1 transition of the permission flag (event).    |
//| Continuous true is NOT a burst; only the rising edge counts.      |
//| Counters increment once per closed bar, not per tick.             |
//+------------------------------------------------------------------+
bool PermissionSilenceTriggered(int basketDirection)
{
    if(basketDirection == BASKET_LONG)
    {
        if(barsSinceLongPermissionBurst >= MAX_PERMISSION_SILENCE_BARS)
            return true;
    }
    else if(basketDirection == BASKET_SHORT)
    {
        if(barsSinceShortPermissionBurst >= MAX_PERMISSION_SILENCE_BARS)
            return true;
    }
...
//+------------------------------------------------------------------+
//| v3.9.2: Update Permission Silence Counters (called per new bar)  |
//|                                                                   |
//| Detects rising-edge burst events (false→true transition).         |
//| Resets counter to 0 on burst, otherwise increments by 1.          |
//| Must be called AFTER UpdateTrendPermission() so that              |
//| longEntryAllowed/Prev are already updated for current bar.        |
//+------------------------------------------------------------------+
void UpdatePermissionSilenceCounters()
{
    // Long burst: permission just turned ON this bar (rising edge)
    if(longEntryAllowed && !longEntryAllowedPrev)
        barsSinceLongPermissionBurst = 0;
    else
        barsSinceLongPermissionBurst++;
    
    // Short burst: permission just turned ON this bar (rising edge)
    if(shortEntryAllowed && !shortEntryAllowedPrev)
        barsSinceShortPermissionBurst = 0;
    else
        barsSinceShortPermissionBurst++;
}
...
void Layer0_RiskCheck()
{
    ...
    if(basketExists)
    {
        // === v3.9.2: PERMISSION SILENCE EXIT (BEFORE EMA30 AND BASKET STOP) ===
        // If no new permission burst for MAX_PERMISSION_SILENCE_BARS closed bars,
        // the trend conviction has faded — exit before structural damage deepens.
        if(PermissionSilenceTriggered(currentDirection))
        {
            if(EnableDebugPrints)
            {
                int posCount = CountBasketPositions();
                int barsSilent = (currentDirection == BASKET_LONG) ? 
                    barsSinceLongPermissionBurst : barsSinceShortPermissionBurst;
                
                Print("!!! LAYER 0: PERMISSION SILENCE EXIT !!!");
                Print("L0 EXIT: Permission Silence",
                      " | Dir: ", currentDirection,
                      " | BarsSilent: ", barsSilent,
                      " | Floating: ", DoubleToString(basketFloatingPL, 2),
                      " | Positions: ", posCount);
            }
            
            CloseAllPositions("Layer0_PermissionSilence");
            ResetBasketState();
            disableReason = "PERMISSION SILENCE EXIT (Ready)";
            return;  // Prevent EMA30, basket stop, or giveback on same tick
        }
        
        // === v3.9.1: STRUCTURAL EMA30 INVALIDATION (BEFORE BASKET STOP) ===
        // Fires on true structural failure: 2 consecutive closes wrong side of EMA30 + slope confirms
        // Prevents large basket stop losses by exiting early when structure is broken
        if(StructuralInvalidationTriggered(currentDirection))
        {
            if(EnableDebugPrints)
            {
                double close1 = iClose(_Symbol, PERIOD_CURRENT, 1);
                ...
                Print("!!! LAYER 0: EMA30 STRUCTURAL INVALIDATION !!!");
                Print("L0 EXIT: EMA30 Structural Invalidation",
                      " | Dir: ", currentDirection,
                      " | Close1: ", DoubleToString(close1, 2),
                      " | Close2: ", DoubleToString(close2, 2),
                      " | EMA30: ", DoubleToString(emaSlow[1], 2),
                      " | Slope: ", DoubleToString(emaFastSlope, 4),
                      " | Floating: ", DoubleToString(basketFloatingPL, 2),
                      " | Positions: ", posCount);
            }
            
            CloseAllPositions("Layer0_EMA30_Structural");
            ResetBasketState();
            disableReason = "EMA30 STRUCTURAL EXIT (Ready)";
            return;  // Prevent basket stop or giveback on same tick
        }
        
        // === PRIORITY: BASKET STOP LOSS ===
        ...
        // === PRIORITY: PEAK PROFIT PROTECTION + v3.9 DAILY FLOOR ===
        ...
        // v3.9: DAILY PROFIT FLOOR EXIT (tighter constraint, checked first)
        ...
        // PEAK PROFIT GIVEBACK (only if daily floor didn't close)
...
void CheckDayReset()
{
    ...
    if(nyDate != currentNYDate)
    {
        ...
        // v3.9.2: Reset silence counters on new NY day
        barsSinceLongPermissionBurst = 0;
        barsSinceShortPermissionBurst = 0;
        ...
        Print("v3.9.2: Permission silence counters reset");
        Print("==============================================");
    }
}
...
void ResetBasketState()
{
    ...
    // v3.9.2: Reset silence counters (basket-scoped — new basket, fresh count)
    barsSinceLongPermissionBurst = 0;
    barsSinceShortPermissionBurst = 0;
    ...
    if(EnableDebugPrints)
        Print(">>> BASKET STATE RESET | Peak: $0.00, Protection: false, Anchor: 0, Silence: 0/0");
}
...
void ProcessNewBar()
{
    ...
    // v3.9.2: Update silence counters AFTER permission values are computed
    // Must be after UpdateTrendPermission() so longEntryAllowed/Prev are current
    UpdatePermissionSilenceCounters();
...
{
    double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v3_9_2_BUY"))
        return true;
...
{
    double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v3_9_2_SELL"))
        return true;
...
void ManageTakeProfit()
{
    ...
    //   - Peak giveback protection (Layer0_RiskCheck)
    //   - Daily profit floor (Layer0_RiskCheck)
...
void ManageGridAdds()
{
    ...
    if(direction == BASKET_LONG)
    {
        ...
        if(bid >= lastAddAnchorPrice + FixedGridGap_USD)
        {
            ...
            double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v3_9_2_GRID_BUY"))
            {
                lastAddAnchorPrice = ask;
                if(EnableDebugPrints)
                {
    ...
    else if(direction == BASKET_SHORT)
    {
        ...
        if(ask <= lastAddAnchorPrice - FixedGridGap_USD)
        {
            double prevAnchor = lastAddAnchorPrice;
            double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v3_9_2_GRID_SELL"))
            {
                lastAddAnchorPrice = bid;
                if(EnableDebugPrints)
                {
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v3.9.2", C'100,180,255', 12, PANEL_FONT_BOLD);
    ...
    // v3.9.2: Increased height from 150 to 176 for silence row
    CreateBox(PREFIX + "ExecContentBG", PANEL_X, y, PANEL_WIDTH, 176, CLR_CONTENT_BG);
    contentY = y + 10;
    ...
    // v3.9.2: Permission silence row
    CreateLabel(PREFIX + "SilenceLabel", PANEL_X + padding, contentY, "Perm Silence:", CLR_LABEL, PANEL_FONT_SIZE, PANEL_FONT);
    CreateLabel(PREFIX + "SilenceValue", valueX, contentY, "", CLR_VALUE, 11, PANEL_FONT_BOLD);
...
void UpdatePanel()
{
    ...
    // v3.9.2: Permission silence display
    string silenceText;
    color silenceColor;
    if(!basketExists)
    {
        silenceText = "○ NO BASKET";
        silenceColor = clrGray;
    }
    else
    {
        int activeSilence = (currentDirection == BASKET_LONG) ? 
            barsSinceLongPermissionBurst : barsSinceShortPermissionBurst;
        silenceText = IntegerToString(activeSilence) + " / " + IntegerToString(MAX_PERMISSION_SILENCE_BARS) + " bars";
        if(activeSilence >= MAX_PERMISSION_SILENCE_BARS)
            silenceColor = clrRed;
        else if(activeSilence >= MAX_PERMISSION_SILENCE_BARS - 1)
            silenceColor = clrOrange;
        else
            silenceColor = clrLime;
    }
    ObjectSetString(0, PREFIX + "SilenceValue", OBJPROP_TEXT, silenceText);
    ObjectSetInteger(0, PREFIX + "SilenceValue", OBJPROP_COLOR, silenceColor);
...
void OnDeinit(const int reason)
{
    DeletePanel();
    Comment("");
    Print("TrendPermissionEA_v3_9_2 deinitialized. Reason: ", reason);
