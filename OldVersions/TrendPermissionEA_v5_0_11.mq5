//+------------------------------------------------------------------+
//|                                      TrendPermissionEA_v5_0_11.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer     |
//|                                                    Version 5.0.11  |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "5.0.11"
#property description "v5.0.11 – C-Architecture Pullback Hedge Overlay"

#include <Trade\Trade.mqh>

//+------------------------------------------------------------------+
//| v5.0.0 COMPILE-TIME FLAGS (Stage 0 scaffolding)                  |
//+------------------------------------------------------------------+
#define OVERLAY_ENABLED  0
// MagicNumber (input) is the single source of truth for Trend Engine magic.
#define MAGIC_OVERLAY    987654
...
input bool   UseDailyProfitFloor    = true;     // v5.0.5: Enable basket floating profit floor
input double BasketProfitFloorTrigger_USD = 50.0; // v5.0.5: Basket peak float to activate floor
input double BasketProfitFloorLevel_USD   = 25.0; // v5.0.5: Floor level — close if float drops to this
...
input group "05. Overlay Engine Settings (v5.0.11 C-Architecture Pullback Hedge)"
input bool   EnableOverlayEngine = false;
input double OverlayLotSize = 0.01;                    // Overlay lot size per position
input double OverlayProfitTarget_USD = 10.0;           // Overlay basket profit target ($)
input double OverlayBasketStopBuffer_USD = 0.0;        // Buffer above/below swing for structural stop ($)
input double OverlayGridGap_USD = 4.0;                 // Overlay grid pyramiding gap ($)
input int    OverlayMaxPositions = 5;                   // Max overlay grid levels
input int    OverlayMaxMinutes = 30;                    // Max overlay basket age (minutes)
input bool   OverlayPullbackMode = true;                // v5.0.11: Use pullback-magnitude activation
input double OverlayPullbackTrigger_USD = 8.0;          // v5.0.11: USD pullback from anchor to trigger overlay
input double OverlayRearmReset_USD = 3.0;               // v5.0.11: Pullback must shrink to this to re-arm
input int    OverlayCooldownSeconds = 60;                // v5.0.11: Cooldown after overlay close (seconds)
input bool   OverlayRequireTrendPermission = true;       // v5.0.11: Require active trend permission for overlay
input bool   OverlayRequireBasketExists = true;          // v5.0.11: Require trend basket to exist for overlay
...
// v5.0.8 CHANGE: Cached indicator handles (created once in OnInit, released in OnDeinit)
...
// ============================
// OVERLAY ENGINE STATE (v5.0.11 C-Architecture Pullback Hedge)
// ============================
bool     overlayActive = false;
int      overlayLevelCount = 0;
double   overlayFloatingPL = 0.0;
double   overlaySwingReference = 0.0;
datetime overlayEntryTime = 0;
datetime overlayLastEntryTickTime = 0;
double   lastOverlayAnchor = 0.0;

// v5.0.11: Pullback hedge state
datetime overlayLastCloseTime = 0;   // v5.0.11: Timestamp of last overlay close (for cooldown)
bool     overlayArmed = true;        // v5.0.11: Armed flag — controlled only by pullback reset logic

// v5.0.4 CHANGE: overlayDailyRealizedPL REMOVED — overlay realized P&L unified into dailyRealizedPL

// Stage 3 v5.0.1: Impulse detection state (retained, calls removed in v5.0.11)
int      overlayPlannedDirection = BASKET_NONE;
bool     overlaySignalActive     = false;
datetime overlaySignalBarTime    = 0;
double   overlayImpulseHigh      = 0.0;
double   overlayImpulseLow       = 0.0;
double   overlayImpulseClose     = 0.0;
bool     overlayImpulseValid     = false;

//+------------------------------------------------------------------+
//| Symbol validation - accepts any XAUUSD variation                 |
//+------------------------------------------------------------------+
bool IsGoldSymbol(string sym)
{
    ...
    string upper = sym;
    StringToUpper(upper);
    // Accept any symbol that starts with "XAUUSD" (covers XAUUSD, XAUUSDm, XAUUSDc, XAUUSD., XAUUSD.raw, etc.)
...
int OnInit()
{
    ...
    {
        Print("ERROR: This EA is designed for XAUUSD (Gold) symbols only. Current symbol: ", _Symbol);
        return(INIT_FAILED);
    }
    ...
    // v5.0.8 CHANGE: Cache all indicator handles at startup
    hEmaHigh = iMA(_Symbol, PERIOD_CURRENT, TrendEMA_Len, 0, MODE_EMA, PRICE_HIGH);
    hEmaLow  = iMA(_Symbol, PERIOD_CURRENT, TrendEMA_Len, 0, MODE_EMA, PRICE_LOW);
    hEmaFast = iMA(_Symbol, PERIOD_CURRENT, FastEMA_Len, 0, MODE_EMA, PRICE_CLOSE);
    hEmaSlow = iMA(_Symbol, PERIOD_CURRENT, SlowEMA_Len, 0, MODE_EMA, PRICE_CLOSE);
    hATR     = iATR(_Symbol, PERIOD_CURRENT, ATR_PERIOD);
    hADX     = iADX(_Symbol, ADX_Timeframe, ADX_Period);
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v5_0_11 initialized");
    Print("Overlay Mode: C-ARCHITECTURE PULLBACK HEDGE");
    Print("Overlay Daily Risk: UNIFIED with Layer 0");
    Print("Basket Floor: Trigger=$", DoubleToString(BasketProfitFloorTrigger_USD, 2), " Level=$", DoubleToString(BasketProfitFloorLevel_USD, 2), " Enabled=", UseDailyProfitFloor);
    Print("State Integrity Guards: ACTIVE");
    Print("Symbol: ", _Symbol, " | Timeframe: M1");
    Print("----------------------------------------------");
    Print("LAYER 0 v5.0.11 - ASYMMETRIC BASKET TP + GRID PYRAMIDING + HTF ADX + PULLBACK OVERLAY");
    ...
    Print("v5.0.11: Overlay PullbackMode=", OverlayPullbackMode, " Trigger=$", DoubleToString(OverlayPullbackTrigger_USD, 2),
          " Rearm=$", DoubleToString(OverlayRearmReset_USD, 2), " Cooldown=", OverlayCooldownSeconds, "s");
    Print("v5.0.11: Overlay RequireTrend=", OverlayRequireTrendPermission, " RequireBasket=", OverlayRequireBasketExists);
    ...
    Print("KEPT: Basket Profit Floor (Trigger=$", DoubleToString(BasketProfitFloorTrigger_USD, 2), ", Level=$", DoubleToString(BasketProfitFloorLevel_USD, 2), ")");
    ...
    Print("OVERLAY_ENABLED: ", OVERLAY_ENABLED);
    Print("MagicNumber: ", MagicNumber, " | MAGIC_OVERLAY: ", MAGIC_OVERLAY);
    Print("==============================================");
...
//+------------------------------------------------------------------+
//| v5.0.0 Stage 1: MAGIC-PARAMETERIZED BASKET UTILITIES             |
//|                                                                  |
//| These generic functions operate on (symbol, magic) and replicate  |
//| the original v3.7+ logic exactly. The original functions are      |
//| preserved below as thin wrappers calling these with MagicNumber.  |
//+------------------------------------------------------------------+

// Returns true if there is at least one open position with matching symbol + magic
bool BasketHasPositionsMagic(long magic)
{
...
int GetBasketDirectionMagic(long magic)
{
    bool hasLong = false;
    bool hasShort = false;
    ...
    else if(hasLong && hasShort)
    {
        ...
        if(EnableDebugPrints)
            Print("!!! v3.7 ASSERTION: Mixed LONG/SHORT positions detected in basket (magic=", magic, ")");
...
// Returns sum of POSITION_PROFIT for all positions matching symbol + magic
double CalculateBasketFloatingPLMagic(long magic)
{
    double totalPL = 0.0;
...
// Closes all positions matching symbol + magic
void CloseAllPositionsMagic(long magic, string reason)
{
    for(int i = PositionsTotal() - 1; i >= 0; i--)
    {
        ulong ticket = PositionGetTicket(i);
...
//+------------------------------------------------------------------+
//| v3.7: DERIVED BASKET STATE FUNCTIONS (thin wrappers)             |
//| v5.0.0: Now delegate to magic-parameterized versions             |
//+------------------------------------------------------------------+

bool BasketHasPositions()
{
    return BasketHasPositionsMagic(MagicNumber);
}

int GetBasketDirection()
{
    return GetBasketDirectionMagic(MagicNumber);
}
...
{
    return CalculateBasketFloatingPLMagic(MagicNumber);
}
...
bool CheckForceFlip()
{
    ...
    if(currentDirection == BASKET_LONG && shortEntryAllowed)
    {
        ...
        // v5.0.3 PATCH 1: Sync overlay close on force flip
        if(EnableOverlayEngine && OverlayHasPositions())
        {
            Print(">>> OVERLAY SYNC CLOSE | Reason: Force flip LONG to SHORT");
            CloseOverlayPositions("Trend_Sync_Close");
            ResetOverlayState();
            overlayLastCloseTime = TimeCurrent();
        }
    ...
    if(currentDirection == BASKET_SHORT && longEntryAllowed)
    {
        ...
        if(EnableOverlayEngine && OverlayHasPositions())
        {
            Print(">>> OVERLAY SYNC CLOSE | Reason: Force flip SHORT to LONG");
            CloseOverlayPositions("Trend_Sync_Close");
            ResetOverlayState();
            overlayLastCloseTime = TimeCurrent();
        }
...
void Layer0_RiskCheck()
{
    ...
    if(dailyRealizedPL <= -DailyLossLimit_USD)
    {
        if(!dailyLossLimitHit)
        {
            ...
            // v5.0.3 PATCH 1: Sync overlay close on daily loss limit
            if(EnableOverlayEngine && OverlayHasPositions())
            {
                Print(">>> OVERLAY SYNC CLOSE | Reason: Daily loss limit");
                CloseOverlayPositions("Trend_Sync_Close");
                ResetOverlayState();
                overlayLastCloseTime = TimeCurrent();
            }
    ...
    if(basketExists)
    {
        if(basketFloatingPL <= -BasketStopLoss_USD)
        {
            ...
            // v5.0.3 PATCH 1: Sync overlay close on trend basket stop
            if(EnableOverlayEngine && OverlayHasPositions())
            {
                Print(">>> OVERLAY SYNC CLOSE | Reason: Trend basket hard stop");
                CloseOverlayPositions("Trend_Sync_Close");
                ResetOverlayState();
                overlayLastCloseTime = TimeCurrent();
            }
        ...
        // v5.0.5 CHANGE: Basket profit floor decoupled from daily target
        // Trigger and level are independent inputs, not tied to DailyProfitTarget_USD
        if(UseDailyProfitFloor && basketPeakFloatingPL >= BasketProfitFloorTrigger_USD && !dailyProfitFloorActive)
        {
            dailyProfitFloorActive = true;
            dailyProfitFloor_USD = BasketProfitFloorLevel_USD;
            if(EnableDebugPrints)
                Print(">>> v5.0.5: BASKET PROFIT FLOOR ACTIVATED at $", DoubleToString(dailyProfitFloor_USD, 2),
                      " | Trigger: $", DoubleToString(BasketProfitFloorTrigger_USD, 2),
                      " | Basket Peak: $", DoubleToString(basketPeakFloatingPL, 2));
        }
        
        // v5.0.5: PRIORITY 3a - BASKET PROFIT FLOOR EXIT (decoupled from daily target)
        if(UseDailyProfitFloor && dailyProfitFloorActive && basketFloatingPL <= BasketProfitFloorLevel_USD)
        {
            Print("!!! v5.0.5: BASKET PROFIT FLOOR EXIT !!!");
            Print("    Floor Level: $", DoubleToString(BasketProfitFloorLevel_USD, 2));
            ...
            CloseAllPositions("Layer0_BasketProfitFloor");
            
            // v5.0.5: Sync overlay close on basket profit floor exit
            if(EnableOverlayEngine && OverlayHasPositions())
            {
                Print(">>> OVERLAY SYNC CLOSE | Reason: Basket profit floor exit");
                CloseOverlayPositions("Layer0_BasketProfitFloor");
                ResetOverlayState();
                overlayLastCloseTime = TimeCurrent();
            }
            ...
            ResetBasketState();
            tradingDisabled = true;
            disableReason = "BASKET FLOOR EXIT";
            return;
        }
        ...
        else if(profitProtectionActivated)
        {
            ...
            if(basketFloatingPL <= givebackThreshold)
            {
                ...
                // v5.0.3 PATCH 1: Sync overlay close on peak giveback
                if(EnableOverlayEngine && OverlayHasPositions())
                {
                    Print(">>> OVERLAY SYNC CLOSE | Reason: Peak giveback");
                    CloseOverlayPositions("Trend_Sync_Close");
                    ResetOverlayState();
                    overlayLastCloseTime = TimeCurrent();
                }
    ...
    if(dailyRealizedPL >= DailyProfitTarget_USD)
    {
        if(!tradingDisabled || disableReason != "DAILY TARGET REACHED")
        {
            ...
            // v5.0.3 PATCH 1: Sync overlay close on daily target
            if(EnableOverlayEngine && OverlayHasPositions())
            {
                Print(">>> OVERLAY SYNC CLOSE | Reason: Daily target reached");
                CloseOverlayPositions("Trend_Sync_Close");
                ResetOverlayState();
                overlayLastCloseTime = TimeCurrent();
            }
...
void CheckDayReset()
{
    ...
    if(nyDate != currentNYDate)
    {
        ...
        // v5.0.11: Re-arm overlay on new day
        overlayArmed = true;
        overlayLastCloseTime = 0;
        ...
        Print("v5.0.5: Basket Profit Floor reset");
        Print("v5.0.11: Overlay armed reset");
        Print("==============================================");
    }
}
...
//+------------------------------------------------------------------+
//| Close All Positions (thin wrapper)                               |
//+------------------------------------------------------------------+
void CloseAllPositions(string reason)
{
    CloseAllPositionsMagic(MagicNumber, reason);
}

//+------------------------------------------------------------------+
//| OVERLAY BASKET UTILITY WRAPPERS (Stage 2)                        |
//+------------------------------------------------------------------+

bool OverlayHasPositions()
{
    return BasketHasPositionsMagic(MAGIC_OVERLAY);
}

int GetOverlayDirection()
{
    return GetBasketDirectionMagic(MAGIC_OVERLAY);
}

double CalculateOverlayFloatingPL()
{
    return CalculateBasketFloatingPLMagic(MAGIC_OVERLAY);
}

void CloseOverlayPositions(string reason)
{
    CloseAllPositionsMagic(MAGIC_OVERLAY, reason);
}

//+------------------------------------------------------------------+
//| v5.0.11: OVERLAY STATE RESET (C-Architecture)                    |
//| NOTE: Does NOT reset overlayArmed — controlled by pullback logic  |
//+------------------------------------------------------------------+
void ResetOverlayState()
{
    overlayLevelCount = 0;
    overlayPlannedDirection = BASKET_NONE;
    overlaySignalActive = false;
    overlayImpulseValid = false;
    overlaySignalBarTime = 0;
    overlayFloatingPL = 0.0;
    overlaySwingReference = 0.0;
    overlayEntryTime = 0;
    overlayLastEntryTickTime = 0;
    lastOverlayAnchor = 0.0;
    // v5.0.11: overlayArmed NOT reset here — controlled only by pullback logic
    
    if(EnableDebugPrints)
        Print(">>> OVERLAY STATE RESET");
}

//+------------------------------------------------------------------+
//| v5.0.11: OVERLAY GRID PYRAMIDING (C-Architecture)                |
//+------------------------------------------------------------------+
void ManageOverlayGrid()
{
    if(!overlayActive)
        return;
    
    // Safety lock — verify positions still exist
    if(!OverlayHasPositions())
        return;
    
    if(overlayLevelCount >= OverlayMaxPositions)
        return;
    if(lastOverlayAnchor == 0.0)
        return;
    
    int ovDir = GetOverlayDirection();
    
    if(ovDir == BASKET_SHORT)
    {
        double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
        if(ask <= lastOverlayAnchor - OverlayGridGap_USD)
        {
            double prevAnchor = lastOverlayAnchor;
            double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            
            trade.SetExpertMagicNumber(MAGIC_OVERLAY);
            if(trade.Sell(OverlayLotSize, _Symbol, bid, 0, 0, "Overlay_v5_0_11_GRID_SELL"))
            {
                overlayLevelCount++;
                lastOverlayAnchor = bid;
                if(EnableDebugPrints)
                    Print(">>> OVERLAY GRID ADD SELL | Price: ", DoubleToString(bid, 2),
                          " | OldAnchor: ", DoubleToString(prevAnchor, 2),
                          " | NewAnchor: ", DoubleToString(lastOverlayAnchor, 2),
                          " | Level: ", overlayLevelCount, "/", OverlayMaxPositions);
            }
            trade.SetExpertMagicNumber(MagicNumber);
        }
    }
    else if(ovDir == BASKET_LONG)
    {
        double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
        if(bid >= lastOverlayAnchor + OverlayGridGap_USD)
        {
            double prevAnchor = lastOverlayAnchor;
            double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            
            trade.SetExpertMagicNumber(MAGIC_OVERLAY);
            if(trade.Buy(OverlayLotSize, _Symbol, ask, 0, 0, "Overlay_v5_0_11_GRID_BUY"))
            {
                overlayLevelCount++;
                lastOverlayAnchor = ask;
                if(EnableDebugPrints)
                    Print(">>> OVERLAY GRID ADD BUY | Price: ", DoubleToString(ask, 2),
                          " | OldAnchor: ", DoubleToString(prevAnchor, 2),
                          " | NewAnchor: ", DoubleToString(lastOverlayAnchor, 2),
                          " | Level: ", overlayLevelCount, "/", OverlayMaxPositions);
            }
...
//+------------------------------------------------------------------+
//| v5.0.11: OVERLAY STRUCTURAL STOP (uses overlaySwingReference)    |
//+------------------------------------------------------------------+
void CheckOverlayStructuralStop()
{
    if(!overlayActive)
        return;
    
    int trendDir = GetBasketDirection();
    
    // v5.0.11: Structural stop based on price returning to swing reference
    // If trend is LONG, overlay is SHORT — close if bid > swingRef (price recovered)
    // If trend is SHORT, overlay is LONG — close if ask < swingRef (price recovered)
    if(trendDir == BASKET_LONG)
    {
        double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
        if(bid > overlaySwingReference + OverlayBasketStopBuffer_USD)
        {
            Print("!!! OVERLAY STRUCTURAL STOP | TrendDir=LONG | Bid=", DoubleToString(bid, 2),
                  " > SwingRef=", DoubleToString(overlaySwingReference, 2),
                  " + Buffer=", DoubleToString(OverlayBasketStopBuffer_USD, 2));
            CloseOverlayPositions("Overlay_Structural_Stop");
            overlayLastCloseTime = TimeCurrent();
            ResetOverlayState();
        }
    }
    else if(trendDir == BASKET_SHORT)
    {
        double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
        if(ask < overlaySwingReference - OverlayBasketStopBuffer_USD)
        {
            Print("!!! OVERLAY STRUCTURAL STOP | TrendDir=SHORT | Ask=", DoubleToString(ask, 2),
                  " < SwingRef=", DoubleToString(overlaySwingReference, 2),
                  " - Buffer=", DoubleToString(OverlayBasketStopBuffer_USD, 2));
            CloseOverlayPositions("Overlay_Structural_Stop");
            overlayLastCloseTime = TimeCurrent();
...
//+------------------------------------------------------------------+
//| v5.0.11: OVERLAY BASKET PROFIT TARGET                            |
//+------------------------------------------------------------------+
void CheckOverlayProfitTarget()
{
    if(!overlayActive)
        return;
    
    if(overlayFloatingPL >= OverlayProfitTarget_USD)
    {
        Print(">>> OVERLAY PROFIT TARGET HIT | Float: $", DoubleToString(overlayFloatingPL, 2),
              " >= Target: $", DoubleToString(OverlayProfitTarget_USD, 2),
              " | Levels: ", overlayLevelCount);
        CloseOverlayPositions("Overlay_Profit_Target");
        overlayLastCloseTime = TimeCurrent();
        ResetOverlayState();
    }
}
...
//+------------------------------------------------------------------+
//| Stage 3 v5.0.1: IMPULSE CANDLE DETECTION (retained, not called)  |
//+------------------------------------------------------------------+
bool DetectImpulseCandle()
{
    // v5.0.11: Function retained but no longer called from OnTick
    if(overlayActive)
        return false;
    
    datetime barTime = iTime(_Symbol, PERIOD_CURRENT, 1);
    
    if(barTime == overlaySignalBarTime)
        return overlayImpulseValid;
    
    overlaySignalBarTime = barTime;
    overlayImpulseValid  = false;
    overlaySignalActive  = false;
    overlayPlannedDirection = BASKET_NONE;
    ...
    overlayImpulseHigh  = h1;
    overlayImpulseLow   = l1;
    overlayImpulseClose = c1;
    overlayImpulseValid = true;
    
    if(EnableDebugPrints)
        Print(">>> v5.0.1 IMPULSE DETECTED | Dir=", (c1 > o1 ? "BULL" : "BEAR"),
...
//+------------------------------------------------------------------+
//| Stage 3 v5.0.1: TICK-LEVEL EXHAUSTION DETECTION (retained, not called) |
//+------------------------------------------------------------------+
bool DetectTickExhaustion(int &direction)
{
    // v5.0.11: Function retained but no longer called from OnTick
    if(!overlayImpulseValid)
        return false;
    if(overlaySignalActive)
        return false;
    
    double o1 = iOpen(_Symbol, PERIOD_CURRENT, 1);
    double c1 = iClose(_Symbol, PERIOD_CURRENT, 1);
    bool isBullishImpulse = (c1 > o1);
    ...
    if(isBullishImpulse)
    {
        ...
        if(h0 > overlayImpulseHigh && bid < overlayImpulseClose)
        {
            double upperWick = h0 - MathMax(o0, bid);
            double lowerWick = MathMin(o0, bid) - l0;
            
            if(upperWick > lowerWick)
            {
                direction = BASKET_SHORT;
                overlaySignalActive = true;
                overlayPlannedDirection = direction;
                return true;
            }
        }
    }
    else
    {
        ...
        if(l0 < overlayImpulseLow && ask > overlayImpulseClose)
        {
            double upperWick = h0 - MathMax(o0, ask);
            double lowerWick = MathMin(o0, ask) - l0;
            
            if(lowerWick > upperWick)
            {
                direction = BASKET_LONG;
                overlaySignalActive = true;
                overlayPlannedDirection = direction;
                return true;
            }
        ...
        if(trans.deal_type == DEAL_TYPE_BUY || trans.deal_type == DEAL_TYPE_SELL)
        {
            if(HistoryDealSelect(trans.deal))
            {
                ...
                if(entry == DEAL_ENTRY_OUT || entry == DEAL_ENTRY_INOUT)
                {
                    ...
                    // v5.0.4 CHANGE: Unified daily P&L — both trend and overlay feed Layer 0
                    if(magic == MagicNumber || magic == MAGIC_OVERLAY)
                    {
                        dailyRealizedPL += profit;
                        string source = (magic == MagicNumber) ? "TREND" : "OVERLAY";
                        if(EnableDebugPrints)
                            Print(">>> ", source, " DEAL CLOSED | Profit: $", DoubleToString(profit, 2), " | Daily Realized P&L: $", DoubleToString(dailyRealizedPL, 2));
                    }
                }
            }
        }
...
void OnTick()
{
    ...
    // v5.0.10 CHANGE: Derive overlayActive from actual positions each tick
    overlayActive = OverlayHasPositions();
    
    // ==========================================================
    // v5.0.11 OVERLAY ENGINE (C-Architecture Pullback Hedge)
    // Must execute BEFORE trend grid adds
    // ==========================================================
    if(EnableOverlayEngine)
    {
        // v5.0.3 PATCH 2: Overlay must respect tradingDisabled
        if(tradingDisabled)
        {
            if(overlayActive || OverlayHasPositions())
            {
                CloseOverlayPositions("Trading_Disabled_Sync");
                overlayLastCloseTime = TimeCurrent();
                ResetOverlayState();
            }
        }
        ...
        {
            // 1. Update overlay floating P&L
            overlayFloatingPL = CalculateOverlayFloatingPL();
            
            // 2. Time expiry (hardened with overlayEntryTime > 0 guard)
            if(overlayActive && overlayEntryTime > 0 &&
               (TimeCurrent() - overlayEntryTime) >= (datetime)(OverlayMaxMinutes * 60))
            {
                Print(">>> OVERLAY TIME EXPIRY | Age=", (TimeCurrent() - overlayEntryTime) / 60,
                      "min | MaxMinutes=", OverlayMaxMinutes);
                CloseOverlayPositions("Overlay_Time_Expiry");
                overlayLastCloseTime = TimeCurrent();
                ResetOverlayState();
            }
            
            // ==========================================================
            // v5.0.11: PULLBACK HEDGE ENTRY LOGIC
            // Replaces impulse/exhaustion/stress-threshold activation
            // ==========================================================
            if(!overlayActive)
            {
                // Cooldown check
                if(TimeCurrent() - overlayLastCloseTime >= OverlayCooldownSeconds)
                {
                    int trendDir = GetBasketDirection();
                    bool basketExists = BasketHasPositions();
                    
                    // Gate: require basket if configured
                    bool basketOK = true;
                    if(OverlayRequireBasketExists && !basketExists)
                        basketOK = false;
                    
                    // Gate: require valid trend direction
                    bool dirOK = (trendDir != BASKET_NONE);
                    
                    // Gate: require trend permission if configured
                    bool trendOK = true;
                    if(OverlayRequireTrendPermission)
                    {
                        if(trendDir == BASKET_LONG)
                            trendOK = longEntryAllowed;
                        else if(trendDir == BASKET_SHORT)
                            trendOK = shortEntryAllowed;
                        else
                            trendOK = false;
                    }
                    
                    if(basketOK && dirOK && trendOK)
                    {
                        // Reference price = last grid anchor (trend basket)
                        double ref = lastAddAnchorPrice;
                        ...
                        double pullback = 0.0;
                        int overlayDir = BASKET_NONE;
                        
                        if(trendDir == BASKET_LONG)
                        {
                            pullback = ref - bid;
                            overlayDir = BASKET_SHORT;
                        }
                        else if(trendDir == BASKET_SHORT)
                        {
                            pullback = ask - ref;
                            overlayDir = BASKET_LONG;
                        }
                        
                        // Re-arm logic: pullback must shrink before next activation
                        if(!overlayArmed)
                        {
                            if(pullback <= OverlayRearmReset_USD)
                            {
                                overlayArmed = true;
                                if(EnableDebugPrints)
                                    Print(">>> v5.0.11 OVERLAY RE-ARMED | Pullback=$", DoubleToString(pullback, 2),
                                          " <= Rearm=$", DoubleToString(OverlayRearmReset_USD, 2));
                            }
                        }
                        
                        // Entry trigger
                        if(overlayArmed && pullback >= OverlayPullbackTrigger_USD)
                        {
                            overlaySwingReference = ref;
                            overlayEntryTime = TimeCurrent();
                            ...
                            if(overlayDir == BASKET_SHORT)
                            {
                                entryOK = trade.Sell(OverlayLotSize, _Symbol, bid, 0, 0, "Overlay_v5_0_11_SELL");
                                entryPrice = bid;
                            }
                            else if(overlayDir == BASKET_LONG)
                            {
                                entryOK = trade.Buy(OverlayLotSize, _Symbol, ask, 0, 0, "Overlay_v5_0_11_BUY");
                                entryPrice = ask;
                            }
                            ...
                            {
                                overlayLevelCount = 1;
                                lastOverlayAnchor = entryPrice;
                                overlayArmed = false;
                                
                                Print(">>> v5.0.11 OVERLAY ENTRY | Dir=", (overlayDir == BASKET_LONG ? "LONG" : "SHORT"),
                                      " | Price=", DoubleToString(entryPrice, 2),
                                      " | SwingRef=", DoubleToString(overlaySwingReference, 2),
                                      " | Pullback=$", DoubleToString(pullback, 2),
                                      " | TrendDir=", (trendDir == BASKET_LONG ? "LONG" : "SHORT"));
                            }
                            else
                            {
                                Print("!!! OVERLAY ENTRY FAILED | Dir=", overlayDir, " | Error=", GetLastError());
                                ResetOverlayState();
                            }
                        }
                    }
            ...
            // Re-derive after potential entry
            overlayActive = OverlayHasPositions();
            overlayFloatingPL = CalculateOverlayFloatingPL();
            
            // 3. Overlay structural stop (must precede grid expansion)
            CheckOverlayStructuralStop();
            
            // Re-derive after potential stop close
            overlayActive = OverlayHasPositions();
            
            // 4. Overlay basket profit target
            overlayFloatingPL = CalculateOverlayFloatingPL();
            CheckOverlayProfitTarget();
            
            // Re-derive after potential target close
            overlayActive = OverlayHasPositions();
            
            // 5. Overlay grid pyramiding (only after stop/target checks pass)
            ManageOverlayGrid();
        }
    }
...
bool CalculateEMAs_Frozen()
{
    ...
    // v5.0.8 CHANGE: Use cached handles instead of creating/releasing each call
    if(CopyBuffer(hEmaHigh, 0, 0, 3, ema200High) < 3 ||
       CopyBuffer(hEmaLow, 0, 0, 3, ema200Low) < 3 ||
       CopyBuffer(hEmaFast, 0, 0, 3, emaFast) < 3 ||
       CopyBuffer(hEmaSlow, 0, 0, 3, emaSlow) < 3 ||
       CopyBuffer(hATR, 0, 0, 3, atrValues) < 3)
        return false;
...
void DetermineEntryPermissions_Frozen()
{
    ...
    double completedOpen = iOpen(_Symbol, PERIOD_CURRENT, 1);
...
bool UpdateHTF_ADX()
{
    ...
    // v5.0.8 CHANGE: Use cached hADX handle
    double adxBuffer[];
    ArraySetAsSeries(adxBuffer, true);

    if(CopyBuffer(hADX, 0, 0, 1, adxBuffer) < 1)
        return false;
...
void ExecuteTradeLogic()
{
    ...
    // v5.0.7 CHANGE: Block new trend basket entries while overlay is active
    if(EnableOverlayEngine && OverlayHasPositions())
    {
        if(EnableDebugPrints)
            Print(">>> v5.0.7 TREND ENTRY BLOCKED | Overlay active, exposure control");
        return;
    }
...
{
    double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_0_11_BUY"))
        return true;
...
{
    double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_0_11_SELL"))
        return true;
...
void CheckPositionStatus()
{
    ...
    hasOpenPosition = BasketHasPositions();
    ...
    if(hadPosition && !hasOpenPosition)
    {
        ...
        // v5.0.3 PATCH 1: Sync overlay close on manual/external trend basket closure
        if(EnableOverlayEngine && OverlayHasPositions())
        {
            Print(">>> OVERLAY SYNC CLOSE | Reason: Trend basket externally closed");
            CloseOverlayPositions("Trend_Sync_Close");
            overlayLastCloseTime = TimeCurrent();
            ResetOverlayState();
        }
...
void ManageGridAdds()
{
    ...
    if(direction == BASKET_LONG)
    {
        ...
        if(bid >= lastAddAnchorPrice + FixedGridGap_USD)
        {
            ...
            double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_0_11_GRID_BUY"))
            {
                lastAddAnchorPrice = ask;
                if(EnableDebugPrints)
                {
    ...
    else if(direction == BASKET_SHORT)
    {
        ...
        if(ask <= lastAddAnchorPrice - FixedGridGap_USD)
        {
            double prevAnchor = lastAddAnchorPrice;
            double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_0_11_GRID_SELL"))
            {
                lastAddAnchorPrice = bid;
                if(EnableDebugPrints)
                {
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v5.0.11", C'100,180,255', 12, PANEL_FONT_BOLD);
    ...
    // === v5.0.11: Overlay Engine Section (expanded for pullback hedge) ===
    y += 155;
    
    CreateBox(PREFIX + "OverlayBG", PANEL_X, y, PANEL_WIDTH, 30, CLR_SECTION_BG);
    CreateLabel(PREFIX + "OverlayTitle", PANEL_X + padding, y + 6, "◈ OVERLAY ENGINE (PULLBACK)", clrWhite, 11, PANEL_FONT_BOLD);
    y += 35;
    
    CreateBox(PREFIX + "OverlayContentBG", PANEL_X, y, PANEL_WIDTH, 116, CLR_CONTENT_BG);
    contentY = y + 10;
    
    CreateLabel(PREFIX + "OvStateLabel", PANEL_X + padding, contentY, "Overlay State:", CLR_LABEL, PANEL_FONT_SIZE, PANEL_FONT);
    CreateLabel(PREFIX + "OvStateValue", valueX, contentY, "", CLR_VALUE, 11, PANEL_FONT_BOLD);
    contentY += rowHeight;
    
    CreateLabel(PREFIX + "OvFloatLabel", PANEL_X + padding, contentY, "Overlay Float P&L:", CLR_LABEL, PANEL_FONT_SIZE, PANEL_FONT);
    CreateLabel(PREFIX + "OvFloatValue", valueX, contentY, "", CLR_VALUE, 11, PANEL_FONT_BOLD);
    contentY += rowHeight;
    
    CreateLabel(PREFIX + "OvLevelsLabel", PANEL_X + padding, contentY, "Overlay Levels:", CLR_LABEL, PANEL_FONT_SIZE, PANEL_FONT);
    CreateLabel(PREFIX + "OvLevelsValue", valueX, contentY, "", CLR_VALUE, 11, PANEL_FONT_BOLD);
    contentY += rowHeight;
    
    CreateLabel(PREFIX + "OvArmedLabel", PANEL_X + padding, contentY, "Armed:", CLR_LABEL, PANEL_FONT_SIZE, PANEL_FONT);
    CreateLabel(PREFIX + "OvArmedValue", valueX, contentY, "", CLR_VALUE, 11, PANEL_FONT_BOLD);
...
void UpdatePanel()
{
    ...
    color dailyFloatColor = floatingPL >= 0 ? C'100,180,100' : C'180,100,100';
    ...
    // === v5.0.11: Overlay Engine Display (Pullback Hedge) ===
    string ovStateText;
    color ovStateColor;
    if(overlayActive)
    {
        ovStateText = "● ACTIVE";
        ovStateColor = clrLime;
    }
    else if(!overlayArmed)
    {
        ovStateText = "○ DISARMED";
        ovStateColor = clrOrange;
    }
    else if(TimeCurrent() - overlayLastCloseTime < OverlayCooldownSeconds && overlayLastCloseTime > 0)
    {
        ovStateText = "◌ COOLDOWN";
        ovStateColor = clrYellow;
    }
    else
    {
        ovStateText = "○ IDLE";
        ovStateColor = clrGray;
    }
    ObjectSetString(0, PREFIX + "OvStateValue", OBJPROP_TEXT, ovStateText);
    ObjectSetInteger(0, PREFIX + "OvStateValue", OBJPROP_COLOR, ovStateColor);
    
    ObjectSetString(0, PREFIX + "OvFloatValue", OBJPROP_TEXT, "$" + DoubleToString(overlayFloatingPL, 2));
    ObjectSetInteger(0, PREFIX + "OvFloatValue", OBJPROP_COLOR, overlayFloatingPL >= 0 ? clrLime : clrRed);
    
    ObjectSetString(0, PREFIX + "OvLevelsValue", OBJPROP_TEXT, IntegerToString(overlayLevelCount) + "/" + IntegerToString(OverlayMaxPositions));
    ObjectSetInteger(0, PREFIX + "OvLevelsValue", OBJPROP_COLOR, CLR_VALUE);
    
    // v5.0.11: Armed display
    string armedText = overlayArmed ? "● ARMED" : "○ DISARMED";
    color armedColor = overlayArmed ? clrLime : clrOrange;
    ObjectSetString(0, PREFIX + "OvArmedValue", OBJPROP_TEXT, armedText);
    ObjectSetInteger(0, PREFIX + "OvArmedValue", OBJPROP_COLOR, armedColor);
...
{
    // v5.0.8 CHANGE: Release cached indicator handles
    ...
    DeletePanel();
    Comment("");
    Print("TrendPermissionEA_v5_0_11 deinitialized. Reason: ", reason);
