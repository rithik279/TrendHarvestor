//+------------------------------------------------------------------+
//|                                        TrendPermissionEA_v3_3.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer    |
//|                                                       Version 3.3 |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "3.3"
#property description "Basket risk based on Floating P&L (not equity)"
...
//+------------------------------------------------------------------+
//| GLOBAL VARIABLES - LAYER 0: RISK GOVERNOR (v3.3 - Floating P&L)  |
//+------------------------------------------------------------------+
...
// === Basket-Level Tracking (v3.3: Based on Floating P&L) ===
...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v3_3 initialized");
    Print("Symbol: ", _Symbol, " | Timeframe: M1");
    Print("Lot Size: ", LotSize);
    Print("Take Profit: $", TakeProfitDollars);
    Print("Magic Number: ", MagicNumber);
    Print("----------------------------------------------");
    Print("LAYER 0 v3.3 - FLOATING P&L BASED BASKET RISK");
...
//|                                                                  |
//| ================================================================ |
//| ===== LAYER 0: GLOBAL RISK GOVERNOR (v3.3 - Floating P&L) ====== |
//| ================================================================ |
//|                                                                  |
//+------------------------------------------------------------------+
...
void InitializeLayer0()
{
    ...
    // === Basket-Level Tracking (v3.3: Floating P&L based) ===
...
//+------------------------------------------------------------------+
//| Calculate Basket Floating P&L (v3.3)                             |
//| Sum of POSITION_PROFIT for all positions with our MagicNumber    |
//+------------------------------------------------------------------+
...
void Layer0_RiskCheck()
{
    ...
    // === v3.3: CALCULATE BASKET FLOATING P&L ===
    basketFloatingPL = CalculateBasketFloatingPL();
    
    // === PRIORITY 2: BASKET-LEVEL STOP LOSS (v3.3: Based on Floating P&L) ===
    // Only affects current basket - does NOT disable future trading
    if(hasOpenPosition && basketActive)
    {
        // v3.3: Basket stop triggers when floating P&L <= -BasketStopLoss_USD
        ...
        // === PRIORITY 3: PEAK PROFIT GIVEBACK CONTROL (v3.3: Based on Floating P&L) ===
        
        // Update peak floating P&L while basket is active
        if(basketFloatingPL > basketPeakPL)
        {
            basketPeakPL = basketFloatingPL;
        }
        
        // v3.3: Profit protection activates when basketPeakPL >= MinProfitToProtect_USD
        if(basketPeakPL >= MinProfitToProtect_USD)
        {
            ...
            // v3.3: Giveback threshold = basketPeakPL - (PeakGivebackFrac * basketPeakPL)
            ...
            if(basketFloatingPL <= givebackThreshold)
            {
                ...
                Print("Giveback Threshold: $", DoubleToString(givebackThreshold, 2));
...
//+------------------------------------------------------------------+
//| Reset Basket State (v3.3: Floating P&L based)                    |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Start New Basket (v3.3: Floating P&L based)                      |
//+------------------------------------------------------------------+
void StartNewBasket()
{
    basketFloatingPL = 0.0;
...
bool OpenBuyTrade()
{
    ...
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v3_3_BUY"))
    {
        return true;
    }
    else
...
bool OpenSellTrade()
{
    ...
    // No SL, No TP set here - TP managed by dollar amount separately
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v3_3_SELL"))
    {
        return true;
    }
    else
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v3.3", C'100,180,255', 12, PANEL_FONT_BOLD);
...
void UpdatePanel()
{
    ...
    {
        basketPLText = "○ NO BASKET";
        basketPLColor = clrGray;
    }
    else
    {
        basketPLText = "$" + DoubleToString(basketFloatingPL, 2) + " / -$" + DoubleToString(BasketStopLoss_USD, 2);
        if(basketFloatingPL >= 0)
            basketPLColor = clrLime;
        else if(basketFloatingPL > -BasketStopLoss_USD * 0.5)
            basketPLColor = clrOrange;
        else
            basketPLColor = clrRed;
    }
    ...
    // === BASKET PEAK P&L (v3.3) ===
    string basketPeakText;
    color basketPeakColor;
    
    if(!basketActive && !hasOpenPosition)
    {
        basketPeakText = "○ NO BASKET";
        basketPeakColor = clrGray;
    }
    else
    {
        basketPeakText = "$" + DoubleToString(basketPeakPL, 2);
        basketPeakColor = basketPeakPL > 0 ? clrLime : CLR_VALUE;
    }
...
void OnDeinit(const int reason)
{
    ...
    Print("TrendPermissionEA_v3_3 deinitialized. Reason: ", reason);
