//+------------------------------------------------------------------+
//|                                        TrendPermissionEA_v4_1.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer    |
//|                                                       Version 4.1 |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "4.1"
#property description "Asymmetric Basket TP + Grid Pyramiding + EMA10 Invalidation + Continuation Reentry"
...
// v4.1: Continuation reentry after PeakGiveback
bool continuationArmed = false;
int  continuationDirection = 0;   // 1 = long, -1 = short
...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v4_1 initialized");
    Print("Symbol: ", _Symbol, " | Timeframe: M1");
    Print("----------------------------------------------");
    Print("LAYER 0 v4.1 - CONTINUATION REENTRY ENABLED");
    Print("NEW: Continuation Reentry after PeakGiveback (EMA10 side only)");
    Print("KEPT: Structural EMA10 Invalidation Exit (v4.0)");
    Print("KEPT: Per-position TP DISABLED, basket-level exits only (v3.9)");
    Print("KEPT: Fixed Grid Pyramiding (gap: $", DoubleToString(FixedGridGap_USD, 2), ") (v3.9)");
    Print("KEPT: Daily Profit Floor at $", DoubleToString(DailyProfitTarget_USD, 2), " (v3.9)");
...
//+------------------------------------------------------------------+
//| v4.0: Structural EMA10 Invalidation Exit                        |
//|       Closes basket when completed bar[1] closes against EMA10   |
//|       This manages trend truth, not P&L risk                     |
//+------------------------------------------------------------------+
bool CheckEMA10Invalidation()
{
    // Must have an active basket
    if(!BasketHasPositions())
        return false;
    
    // Need EMA data (emaFast = EMA10)
    ...
    // LONG basket: invalidate if Close[1] < EMA10[1]
    if(direction == BASKET_LONG && completedClose < emaFast[1])
    {
        Print("!!! v4.0: EMA10 INVALIDATION EXIT (LONG) !!!");
        Print("    Close[1]: ", DoubleToString(completedClose, 2),
              " < EMA10[1]: ", DoubleToString(emaFast[1], 2));
        ...
        CloseAllPositions("Layer0_EMA10_Invalidation");
        ResetBasketState();
        disableReason = "EMA10 INVALIDATION (Ready)";
        return true;
    }
    
    // SHORT basket: invalidate if Close[1] > EMA10[1]
    if(direction == BASKET_SHORT && completedClose > emaFast[1])
    {
        Print("!!! v4.0: EMA10 INVALIDATION EXIT (SHORT) !!!");
        Print("    Close[1]: ", DoubleToString(completedClose, 2),
              " > EMA10[1]: ", DoubleToString(emaFast[1], 2));
        Print("    Basket Float: $", DoubleToString(basketFloatingPL, 2),
              " | Basket Peak: $", DoubleToString(basketPeakFloatingPL, 2));
...
void Layer0_RiskCheck()
{
    ...
    // === PRIORITY 2: v4.0 EMA10 STRUCTURAL TREND INVALIDATION ===
    // Exits before P&L stops — manages trend truth, not risk
    if(CheckEMA10Invalidation())
        return;
    
    // === CALCULATE BASKET FLOATING P&L ===
    basketFloatingPL = CalculateBasketFloatingPL();
    
    // === PRIORITY 3: BASKET STOP LOSS ===
    ...
    if(basketExists)
    {
        ...
        // === PRIORITY 4: PEAK PROFIT PROTECTION + v3.9 DAILY FLOOR ===
        ...
        // v3.9: PRIORITY 4a - DAILY PROFIT FLOOR EXIT (tighter constraint, checked first)
        ...
        // PRIORITY 4b: PEAK PROFIT GIVEBACK (only if daily floor didn't close)
        else if(profitProtectionActivated)
        {
            ...
            if(basketFloatingPL <= givebackThreshold)
            {
                ...
                // v4.1: Arm continuation BEFORE closing (capture direction)
                continuationArmed = true;
                continuationDirection = currentDirection;
                if(EnableDebugPrints)
                    Print(">>> v4.1: CONTINUATION ARMED | Direction: ", 
                          continuationDirection == BASKET_LONG ? "LONG" : "SHORT");
...
void CheckDayReset()
{
    ...
    if(nyDate != currentNYDate)
    {
        ...
        // v4.1: Reset continuation on new day
        continuationArmed = false;
        continuationDirection = 0;
...
void OnTick()
{
    ...
    // v4.1: Continuation reentry (tick-based, after risk check, before grid)
    TryContinuationReentry();
...
//+------------------------------------------------------------------+
//| v4.1: Continuation Reentry after PeakGiveback                   |
//|       Tick-based, EMA10 side only, no structural permission      |
//+------------------------------------------------------------------+
void TryContinuationReentry()
{
    ...
    if(ArraySize(emaFast) < 1)
        return;
    
    if(continuationDirection == BASKET_LONG)
    {
        ...
        if(bid > emaFast[0])
        {
            ...
            {
                continuationArmed = false;
                lastAddAnchorPrice = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
                tradeTakenThisPermission = true;
                if(EnableDebugPrints)
                    Print(">>> v4.1 CONTINUATION BUY OPENED | Bid: ", DoubleToString(bid, 2),
                          " > EMA10[0]: ", DoubleToString(emaFast[0], 2),
                          " | Anchor: ", DoubleToString(lastAddAnchorPrice, 2));
            }
        }
    }
    else if(continuationDirection == BASKET_SHORT)
    {
        ...
        if(ask < emaFast[0])
        {
            ...
            {
                continuationArmed = false;
                lastAddAnchorPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
                tradeTakenThisPermission = true;
                if(EnableDebugPrints)
                    Print(">>> v4.1 CONTINUATION SELL OPENED | Ask: ", DoubleToString(ask, 2),
                          " < EMA10[0]: ", DoubleToString(emaFast[0], 2),
                          " | Anchor: ", DoubleToString(lastAddAnchorPrice, 2));
            }
        }
...
{
    double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v4_1_BUY"))
        return true;
...
{
    double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v4_1_SELL"))
        return true;
...
void ManageTakeProfit()
{
    ...
    //   - v4.0: EMA10 trend invalidation (Layer0_RiskCheck PRIORITY 2)
    //   - Peak giveback protection (Layer0_RiskCheck PRIORITY 4b)
    //   - Daily profit floor (Layer0_RiskCheck PRIORITY 4a)
...
void ManageGridAdds()
{
    ...
    if(direction == BASKET_LONG)
    {
        ...
        if(bid >= lastAddAnchorPrice + FixedGridGap_USD)
        {
            ...
            double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v4_1_GRID_BUY"))
            {
                lastAddAnchorPrice = ask;
                if(EnableDebugPrints)
                {
    ...
    else if(direction == BASKET_SHORT)
    {
        ...
        if(ask <= lastAddAnchorPrice - FixedGridGap_USD)
        {
            double prevAnchor = lastAddAnchorPrice;
            double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v4_1_GRID_SELL"))
            {
                lastAddAnchorPrice = bid;
                if(EnableDebugPrints)
                {
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v4.1", C'100,180,255', 12, PANEL_FONT_BOLD);
    ...
    // v4.1: Increased height from 150 to 176 for continuation row
    ...
    // v4.1: Continuation status row
    CreateLabel(PREFIX + "ContLabel", PANEL_X + padding, contentY, "Continuation:", CLR_LABEL, PANEL_FONT_SIZE, PANEL_FONT);
    CreateLabel(PREFIX + "ContValue", valueX, contentY, "", CLR_VALUE, 11, PANEL_FONT_BOLD);
...
void UpdatePanel()
{
    ...
    // v4.1: Continuation status display
    string contText;
    color contColor;
    if(continuationArmed)
    {
        contText = "● ARMED (" + (continuationDirection == BASKET_LONG ? "LONG" : "SHORT") + ")";
        contColor = C'255,200,50';  // Gold/yellow for armed
    }
    else
    {
        contText = "○ INACTIVE";
    ...
    ObjectSetString(0, PREFIX + "ContValue", OBJPROP_TEXT, contText);
    ObjectSetInteger(0, PREFIX + "ContValue", OBJPROP_COLOR, contColor);
...
void OnDeinit(const int reason)
{
    DeletePanel();
    Comment("");
    Print("TrendPermissionEA_v4_1 deinitialized. Reason: ", reason);
