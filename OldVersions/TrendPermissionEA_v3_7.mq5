//+------------------------------------------------------------------+
//|                                        TrendPermissionEA_v3_7.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer    |
//|                                                       Version 3.7 |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "3.7"
#property description "Correctness Patch: Realized P&L accounting, derived basket state"
...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v3_7 initialized");
    Print("Symbol: ", _Symbol, " | Timeframe: M1");
    Print("----------------------------------------------");
    Print("LAYER 0 v3.7 - CORRECTNESS PATCH");
    Print("FIX 1: Daily P&L tracked ONLY in OnTradeTransaction");
    Print("FIX 2: Daily limits use REALIZED P&L only");
    Print("FIX 3: Basket state is DERIVED, not flag-based");
    Print("----------------------------------------------");
...
void ResetBasketState()
{
    ...
    if(EnableDebugPrints)
        Print(">>> BASKET STATE RESET | Peak: $0.00, Protection: false");
}
...
void ExecuteTradeLogic()
{
    ...
    if(longEntryAllowed)
    {
        if(OpenBuyTrade())
        {
            ...
            // v3.7: No need to call StartNewBasket - state is derived
            if(EnableDebugPrints)
...
bool OpenBuyTrade()
{
    ...
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v3_7_BUY"))
        return true;
...
bool OpenSellTrade()
{
    ...
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v3_7_SELL"))
        return true;
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v3.7", C'100,180,255', 12, PANEL_FONT_BOLD);
...
void OnDeinit(const int reason)
{
    DeletePanel();
    Comment("");
    Print("TrendPermissionEA_v3_7 deinitialized. Reason: ", reason);
