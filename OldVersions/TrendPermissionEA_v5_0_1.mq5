//+------------------------------------------------------------------+
//|                                      TrendPermissionEA_v5_0_1.mq5 |
//|                        Layer 0 Risk Governor + Execution Layer    |
//|                                                     Version 5.0.1 |
//+------------------------------------------------------------------+
#property copyright "TrendPermission"
#property version   "5.0.1"
#property description "v5.0.1 – Tick-Level Exhaustion Detection (No Execution)"
...
input group "05. Overlay Engine Settings (Stage 2 - Scaffolding Only)"
input bool   EnableOverlayEngine = false;
input double OverlayActivationThreshold_USD = 20.0;
input double OverlayProfitTarget_USD = 10.0;
input double OverlayGridGap_USD = 4.0;
input int    OverlayMaxPositions = 5;
...
// ============================
// OVERLAY ENGINE STATE (Stage 2 - No Execution)
// ============================
...
int OnInit()
{
    ...
    Print("==============================================");
    Print("TrendPermissionEA_v5_0_1 initialized");
    Print("Symbol: ", _Symbol, " | Timeframe: M1");
    Print("----------------------------------------------");
    Print("LAYER 0 v5.0.1 - ASYMMETRIC BASKET TP + GRID PYRAMIDING + HTF ADX");
...
void OnTick()
{
    ...
    // Stage 2: Update overlay state (display only, no execution)
    ...
    // Stage 3 v5.0.1: Impulse detection (once per new bar)
    if(EnableOverlayEngine)
    {
        DetectImpulseCandle();
    }
    ...
    if(EnableOverlayEngine && !overlayActive)
    {
        ...
        if(DetectTickExhaustion(exDirection))
        {
            ...
            Print(">>> v5.0.1 EXHAUSTION SIGNAL | Direction=", exDirection,
                  " | Ref=", DoubleToString(overlaySwingReference, 2));
        }
    }
...
{
    double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_0_1_BUY"))
        return true;
...
{
    double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_0_1_SELL"))
        return true;
...
void ManageGridAdds()
{
    ...
    if(direction == BASKET_LONG)
    {
        ...
        if(bid >= lastAddAnchorPrice + FixedGridGap_USD)
        {
            ...
            double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            if(trade.Buy(LotSize, _Symbol, ask, 0, 0, "TrendPerm_v5_0_1_GRID_BUY"))
            {
                lastAddAnchorPrice = ask;
                if(EnableDebugPrints)
                {
    ...
    else if(direction == BASKET_SHORT)
    {
        ...
        if(ask <= lastAddAnchorPrice - FixedGridGap_USD)
        {
            double prevAnchor = lastAddAnchorPrice;
            double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
            if(trade.Sell(LotSize, _Symbol, bid, 0, 0, "TrendPerm_v5_0_1_GRID_SELL"))
            {
                lastAddAnchorPrice = bid;
                if(EnableDebugPrints)
                {
...
void CreatePanel()
{
    ...
    CreateLabel(PREFIX + "Version", PANEL_X + 270, y + 14, "v5.0.1", C'100,180,255', 12, PANEL_FONT_BOLD);
...
void OnDeinit(const int reason)
{
    DeletePanel();
    Comment("");
    Print("TrendPermissionEA_v5_0_1 deinitialized. Reason: ", reason);
